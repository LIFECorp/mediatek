...
#define IDX_PS_OFF  3  
    ...
    if (On) {
        ...
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV9726_MIPI_RAW,currSensorName)))
		{
	      ...
	      else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YX_2LANE_MIPI_RAW, currSensorName)))
        {
            //S5K4H5YX_2LANE Power UP
         #if 1
		
		 	
             PK_DBG("[CAMERA SENSOR] Fail to enable digital power ddddddddddddd\n");
       		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                //PDN pin
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN], pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! \n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMPDN], GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! \n");}
			...
			//Power down pin is high	  
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) 
				{ 		   
				if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}			 
				if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}					   
				if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_ON])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}			  	  
				PK_DBG("Power pin is High\n");
