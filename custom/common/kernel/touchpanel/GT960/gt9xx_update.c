...
u16 show_len;
u16 total_len;
static u8 gup_update_use_default_fw = 0;
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
    ...
    else
    {
        ...
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++)
        {
            //check driver default hw
            if(gup_update_use_default_fw)
            {
                GTP_DEBUG("Update use driver default fw");
                gup_update_use_default_fw = 0;
                break;
            } 
...
download:
    ...
    tpd_halt = 1;
    gtp_reset_guitar(i2c_client_point, 20);                        //20121212
    ret = gup_get_ic_fw_msg(i2c_client_point);
    ...
    ret = gup_enter_update_mode(i2c_client_point);                 //20121212
    ...
    while (retry++ < 5)
    {
        ...
        ret = gup_download_fw_ss51(i2c_client_point);
        ...
        ret = gup_download_fw_dsp(i2c_client_point);
    ...
    //-- clear control flag
    GTP_DEBUG("[burn_fw_ss51]step3:clear control flag");
    ret = gup_set_ic_msg(i2c_client_point, _rRW_MISCTL__BOOT_CTL_, 0xAA);
    ...
    ret = gup_set_ic_msg(i2c_client_point, _rRW_MISCTL__SWRST_B0_, 0x00);              //20130307
    ...
    gup_get_ic_msg(i2c_client_point, GUP_REG_FW_MSG, buf, 1);
