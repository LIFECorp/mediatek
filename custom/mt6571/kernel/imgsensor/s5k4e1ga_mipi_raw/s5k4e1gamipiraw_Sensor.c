...
#define S5K4E1GA_TEST_PATTERN_CHECKSUM (0xcbf2fe35)
//****************************** -FUNCTION DECLARATION ******************************//
...
void S5K4E1GA_write_shutter(kal_uint16 shutter)
{
	...
	unsigned long flags;
	SENSORDB("[Lin]shutter=%d", shutter);
	...
	if(s5k4e1ga.S5K4E1GAAutoFlickerMode == KAL_TRUE)
	{
		...
		SENSORDB("[Lin]framerate=%d", framerate);
		...
		if (shutter > max_shutter)
			extra_lines = shutter - max_shutter + 16;//modify shutter range
		else
			extra_lines = 0;
		...
		if ( SENSOR_MODE_PREVIEW == s5k4e1ga.sensorMode )	//SXGA size output
		{
			line_length = S5K4E1GA_PV_PERIOD_PIXEL_NUMS + s5k4e1ga.DummyPixels; 
			frame_length = S5K4E1GA_PV_PERIOD_LINE_NUMS+ s5k4e1ga.DummyLines + extra_lines ; 
			framerate = (s5k4e1ga.pvPclk* 100000) /line_length/frame_length;
		}
		else				//QSXGA size output
		{
			line_length = S5K4E1GA_FULL_PERIOD_PIXEL_NUMS + s5k4e1ga.DummyPixels; 
			frame_length = S5K4E1GA_FULL_PERIOD_LINE_NUMS + s5k4e1ga.DummyLines + extra_lines ; 
			framerate = (s5k4e1ga.capPclk* 100000) /line_length/frame_length;
		}
		SENSORDB("[Lin]else framerate=%d", framerate);
	}
	...
	s5k4e1ga.maxExposureLines = frame_length - 16;
...
void S5K4E1GACaptureSetting(void)
{
    ...
    */
    ...
    //else
    //{
    //S5K4E1GA_write_cmos_sensor(0x0340, 0x07);//2014
    //S5K4E1GA_write_cmos_sensor(0x0341, 0xB4);//2014
    //}
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	{
		SENSORDB("[Lin]S5K4E1GAPreview exit!");
		return ERROR_NONE;
	}
