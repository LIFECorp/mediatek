...
init(int bufwidth,int bufheight)
{
    ...
    if(!(allocMem(imgParam,mpMAVMotionBuffer)))
    {
        MY_LOGE("[init] mpMAVMotionBuffer alloc fail");
        ret = MFALSE;
        goto lb_Abnormal_Exit;
    }
    imgParam.imgSize = mMAVFrameWidth * mMAVFrameHeight * 2 + 2048;
  
    if(!(allocMem(imgParam,mpWarpBuffer)))
    {
    ...
    if(WokeSize==0)
        imgParam.imgSize = mMAVFrameWidth * mMAVFrameHeight * 4 * 10;    
    else
    	  imgParam.imgSize = WokeSize;
        
    if(!(allocMem(imgParam,mpMAVWorkingBuf)))
    {
