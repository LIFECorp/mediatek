...
#include "utils/Format.h"     // REMOVE ME
...
public:     ////                    Image Attributes.
    ...
    virtual MUINT32                 getBitstreamSize()                  const   = 0;
    virtual MBOOL                   setBitstreamSize(MUINT32 const bitstreamsize)  = 0;
...
public:     ////                    Buffer Attributes.
     ...
     */
    virtual MINT32                  getFD(MUINT index = 0)             const   = 0;

    /**
     * Legal only after lockBuf().
     */
    virtual MINT32                  getFDCount()                       const   = 0;
     ...
     */
    virtual MUINT32                 getBufStridesInBytes(MUINT index)   const   = 0;
...
public:     ////                    Params for Allocations.
                                    ...
                                    MUINT32 bufSize;                // buffer size in bytes. for JPEG memory.
                                    MUINT32 bufStridesInBytes[3];   // buffer strides(width) in bytes.
                                    ...
                                    MINT32  bufBoundaryInBytes[3];  // the address will be a multiple of boundary in bytes, which must be a power of two.
                                            ...
                                            ImgParam(
                                                ...
                                                , bufSize(0)
                                            {
                                                CAM_LOGW("[WARNING] to be removed.");
#define PIXEL2BYTE(PLANE, PIXEL, FORMAT)                                        \
    (PIXEL * Utils::Format::queryPlaneBitsPerPixel(FORMAT, PLANE)) >> 3         \

                                                for (MUINT i = 0; i < _planeCount; i++)
                                                {
                                                    bufStridesInBytes[i]  = PIXEL2BYTE(i, _bufStridesInPixels[i].w, imgFormat);
                                            ...
                                            ImgParam(
                                                MINT32 const    _imgFormat, 
                                                MSize const&    _imgSize, 
                                                MUINT32 const   _bufStridesInBytes[], 
                                                MINT32 const    _bufBoundaryInBytes[], 
                                                MUINT const     _planeCount
                                            )
                                                : imgFormat(_imgFormat)
                                                ...
                                                , bufSize(0)
                                            {
                                                for (MUINT i = 0; i < _planeCount; i++)
                                                {
                                                    ...
                                                    bufStridesInPixels[i] = MSize(0, 0);    // REMOVE ME
                                                    bufBoundaryInBytes[i] = _bufBoundaryInBytes[i];
                                                }
                                            }
                                            //
                                            //  BLOB memory.
                                            ImgParam(
                                                MUINT32 const   _bufSize,               // buffer size in bytes. 
