...
static int packStrings(Parcel &p, void *cmd, size_t cmdlen) {
    ...
    if (cmd == NULL && cmdlen != 0) {
        RLOGE("RilUtil: invalid cmd NULL");
        return -1;
    }
    if (cmdlen % sizeof(char *) != 0) {
        RLOGE("RilUtil: invalid cmd length %d expected multiple of %d\n",
...
invalid:
	RLOGE("RilUtil: unpackStrings() fail\n");
    return NULL;
}
...
char * unpackRILResponseFull(char *inRsp, size_t length, int32_t *out_ril_errno, int32_t *out_token)
{
    ...
    if (type == RESPONSE_UNSOLICITED) {
        ...
        RLOGD("unpackRILResponse: Discard UNSOLICITED response:%d\n",token);
        return NULL;
    }
    ...
    if (out_ril_errno != NULL) {
        ...
        *out_ril_errno = e;
        RLOGD("unpackRILResponse: Return error cause: %d", e);
    }
    if (RIL_E_SUCCESS != e) {
