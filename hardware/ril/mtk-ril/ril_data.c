...
extern int definePdnCtx(const char *requestedApn, int protocol, int availableCid, int authType, const char* username, const char* password, RILChannelCtx *pChannel);
...
void requestSetInitialAttachApn(void * data, size_t datalen, RIL_Token t)
{
    ...
    int attachSupport = getAttachApnSupport();
    LOGD("IA: requestSetInitialAttachApn [apn=%s, protocol=%s, auth_type=%d, username=%s, password=%s], attachApnSupport=%d, isCid0Support=%d",
        param->apn, param->protocol, param->authtype, param->username, "XXXXX"/*param->password*/, attachSupport, isCid0Support);

    if (isCid0Support && attachSupport != ATTACH_APN_NOT_SUPPORT) {
        ...
        char* protocolStr;
        if (param->protocol == IPV4V6)
            protocolStr = SETUP_DATA_PROTOCOL_IP;
        else if (param->protocol == IPV6)
            protocolStr = SETUP_DATA_PROTOCOL_IPV6;
        else
            protocolStr = SETUP_DATA_PROTOCOL_IP;
        ...
        if (strlen(iaProperty) != 0) {
            ...
            char iaParameter[PROPERTY_VALUE_MAX] = {0};
            if (strlen(param->password) == 0) {
                sprintf(iaParameter, "%s,%s,%d,%s,%s", iccid, protocolStr, param->authtype, param->username, param->apn);
            } else {
                //when password is set, iccid is not recorded
                //so we do not need to compare iccid
                sprintf(iaParameter, "%s,%s,%d,%s,%s", "", protocolStr, param->authtype, param->username, param->apn);
            }
            ...
            if (strcmp(iaParameter, iaProperty) != 0) {
                ...
                if (strlen(iccid) > 0) {
                    requestDetachPS(1, 1, t); //the 1st & 2nd parameters are used to prevent response is sent
        ...
        if (strlen(iaProperty) == 0) {
            if (!definePdnCtx(param->apn, param->protocol, 0, param->authtype, param->username, param->password, DATA_CHANNEL_CTX)) {
                LOGE("IA: requestSetInitialAttachApn define CID0 failed");
                goto error;
            }

#ifndef MTK_GEMINI
            //The attach is only trigered in single SIM load
            ...
            if (strlen(param->password) == 0) {
                ...
                sprintf(iaProperty, "%s,%s,%d,%s,%s", iccid, protocolStr, param->authtype, param->username, param->apn);
                LOGD("IA: initial attach success, set ia property [%s]", iaProperty);
                property_set(PROPERTY_IA, iaProperty);
            } else {
                LOGD("IA: initial attach APN contain password, set patch to invalid value");
                sprintf(iaProperty, "%s,%s,%d,%s,%s", "", protocolStr, param->authtype, param->username, param->apn);
                property_set(PROPERTY_IA, iaProperty);
            }
        } else {
            LOGD("IA: valid IA property is set, not to do attach again");
        }
    } else {
        LOGD("IA: requestSetInitialAttachApn but CID0 is not support or attach apn is not support");
    }
