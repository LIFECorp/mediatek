...
extern void bootupGetBasebandVersion(RILId rid) {
    ...
    if (getMappingSIMByCurrentMode(rid) == GEMINI_SIM_2){
        err = property_set("gsm.version.baseband.2",s_basebandVersion[rid]);
        if(err < 0) LOGE("SystemProperty: PROPERTY_VERSION_2 set fail");					
    }else{
        err = property_set("gsm.version.baseband" ,s_basebandVersion[rid]);
        if(err < 0) LOGE("SystemProperty: PROPERTY_VERSION set fail");
    } 
#endif
