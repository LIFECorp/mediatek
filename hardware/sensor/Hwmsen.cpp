...
int Hwmsen::enable(int32_t handle, int en) {
	...
	if( handle != ID_ACCELEROMETER && handle != ID_MAGNETIC &&
		...
		handle != ID_PRESSURE && handle != ID_TEMPRERATURE
		)
	{
...
//ganping.li
		case ID_PRESSURE:
			 ...
			 mPendingEvents[pressure].pressure = (float)sensors_data.data[ID_PRESSURE].values[0];
		...
		case ID_TEMPRERATURE:
			 ...
			 mPendingEvents[temperature].temperature = (float)sensors_data.data[ID_TEMPRERATURE].values[0];
...
int Hwmsen::setDelay(int32_t handle, int64_t ns)
{
	...
	if( handle != ID_ACCELEROMETER && handle != ID_MAGNETIC &&
		handle != ID_ORIENTATION && handle != ID_GYROSCOPE && 
		handle != ID_LIGHT && handle != ID_PRESSURE && handle != ID_TEMPRERATURE)
	{
