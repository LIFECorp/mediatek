...
public class CapabilityApi extends ICapabilityRemoteListener.Stub implements RemoteRcseApi {
    ...
    public void onCapabilityChanged(String contact, final Capabilities capabilities)
            throws RemoteException {
        final String  number = contact;
        Logger.v(TAG, "onCapabilityChanged() entry, contact = " + number);
        for (final ICapabilityListener listener : mListeners) {
			
            	Handler refresh = new Handler(Looper.getMainLooper());
            	refresh.post(new Runnable() {
            	    public void run()
            	    {
                   	 	Logger.w(TAG, "onCapabilitiesChanged  capabilityApi: " + number);
                		listener.onCapabilityChanged(number, capabilities);
            }
            	});
