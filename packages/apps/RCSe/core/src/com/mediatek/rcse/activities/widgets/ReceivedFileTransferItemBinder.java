...
public class ReceivedFileTransferItemBinder extends AbsItemBinder implements OnClickListener {
    ...
    private View bindReceiveInvitationView(View itemView) {
        ...
        TextView remoteNameView = (TextView) itemView.findViewById(R.id.remote_name);
        ...
        if (PhoneUtils.isANumber(phoneNum)) {
            ...
            Logger.e(TAG, "bindReceiveInvitationView() the remoteContact " + phoneNum
        ...
        {
            phoneNum = ((One2OneChatFragment.ReceivedFileTransfer)mFile).getContactNum();
        if (phoneNum != null) {
    ...
    private void bindCancelView(View itemView) {
        ...
        if(mFile instanceof One2OneChatFragment.ReceivedFileTransfer)
        {
            stringBuilder.append(((One2OneChatFragment.ReceivedFileTransfer)mFile).getContactName());
        stringBuilder.append(WHITE_SPACE);
        }
        stringBuilder.append(context.getString(R.string.file_transfer_at));
        stringBuilder.append(WHITE_SPACE);
    ...
    private void onUserAccept(Context context) {
        ...
        if (fileSize >= warningFileSize && warningFileSize != 0) {
            ...
            if (isRemind) {
                ...
                if(mFile instanceof One2OneChatFragment.ReceivedFileTransfer)
                {
                    intent.putExtra(RcsNotification.CONTACT, ((One2OneChatFragment.ReceivedFileTransfer)mFile).getContactNum());
                }
