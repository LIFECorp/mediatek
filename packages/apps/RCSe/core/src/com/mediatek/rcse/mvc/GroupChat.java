...
public class GroupChat extends ChatImpl {
    ...
    private void sendInvite(String subject , int messageTag) {
        ...
        if (inviterSize > 0) {
            try {
                IChatSession chatSession = mMessagingApi.initiateAdhocGroupChatSession(pList,subject);
...
//		else {
    ...
    protected void reloadMessage(final InstantMessage message, final int messageType,
            final int status, final int messageTag) {
	if(ismInvite() == false)
	{
    ...
    public void handleAcceptFileTransfer(Object fileTransferTag) {
        ...
        Logger.d(TAG,
                "handleAcceptFileTransfer  group entry(): receiveFileTransfer = "
