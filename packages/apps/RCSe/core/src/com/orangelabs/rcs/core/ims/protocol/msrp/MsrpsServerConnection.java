 ...
 * @author jexa7410
 */
public class MsrpsServerConnection extends MsrpConnection {
    /**
     ...
     */
    public MsrpsServerConnection(MsrpSession session, int localPort) {
        super(session);
    ...
    public SocketConnection getSocketConnection() throws IOException {
        if (mLogger.isActivated()) {
            mLogger.debug("Open server ssl socket at " + mLocalPort);
        }
        SocketServerConnection socketServer = NetworkFactory.getFactory()
                .createSSLSocketServerConnection();
        socketServer.open(mLocalPort);

        if (mLogger.isActivated()) {
            mLogger.debug("Wait client connection");
        }
