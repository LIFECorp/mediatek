...
public class RichMessaging {
	...
	if (logger.isActivated()) {
                    logger.debug("addIncomingChatSession() alias name: " + name);
                }
...
EventsLogApi.EVENT_INVITED);
 ...
 /**
	...
	public void addOutgoingGroupFileTransfer(List<String> contactList, String chatSessionId, String ftSessionId, String fileName, MmContent content){
		// Add session entry
		addEntry(EventsLogApi.TYPE_OUTGOING_FILE_TRANSFER, ftSessionId, chatSessionId, ftSessionId, contactList, fileName, content.getEncoding(), content.getName(), content.getSize(), null, EventsLogApi.EVENT_INITIATED);	
	}
