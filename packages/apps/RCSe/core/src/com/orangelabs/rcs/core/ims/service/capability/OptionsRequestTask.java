...
public class OptionsRequestTask implements Runnable {
	...
	private void handleUserNotRegistered(SipTransactionContext ctx) {
        ...
        if (info.getRcsStatus() == ContactInfo.NO_INFO) {
        	...
        	Capabilities capabilities = new Capabilities();
			if (logger.isActivated()) {
                    logger.info("handleUserNotRegistered No prior info present" + contact);
                }
            /**
            ...
            if (info.getRcsStatus() == ContactInfo.NOT_RCS) {
                ...
                if (logger.isActivated()) {
                    logger.info("handleUserNotRegistered setRcseContact contact: " + contact
                            + " false");
                }
				/**
	...
	private void handleLocalTimeOut(SipTransactionContext ctx) {
        ...
        if (info.getRcsStatus() == ContactInfo.NO_INFO) {
            ...
            ContactsManager.getInstance().setContactCapabilities(contact, capabilities,
                    info.getRcsStatus(), ContactInfo.REGISTRATION_STATUS_UNKNOWN);
			if (logger.isActivated()) {
                    logger.info("handleLocalTimeOut NO_INFO");
                }
		// Update capability timestamp
    	ContactsManager.getInstance().setContactCapabilitiesTimestamp(contact, System.currentTimeMillis());
			// Notify listener
        imsModule.getCore().getListener().handleCapabilitiesNotification(contact,
         ...
         if (logger.isActivated()){
            logger.info("handleLocalTimeOut Status:" + info.getRcsStatus() + "Registration status" + info.getRegistrationState());
			
			if(info.getCapabilities().isImSessionSupported()){
				logger.info("handleLocalTimeOut Rcse true");
			}else{
				logger.info("handleLocalTimeOut Rcse false");
        }
			}
		 // Update capability timestamp
    	ContactsManager.getInstance().setContactCapabilitiesTimestamp(contact, System.currentTimeMillis());
	...
	*/
	...
	private void handleUserNotFound(SipTransactionContext ctx) {
        ...
        if (logger.isActivated()) {
                logger.info("handleUserNotFound Status:" + info.getRcsStatus() + "Registration status" + info.getRegistrationState());
				
				if(info.getCapabilities().isImSessionSupported()){
					logger.info("handleUserNotFound Rcse true");
				}else{
					logger.info("handleUserNotFound Rcse false");
				}
         	}
		// Update capability timestamp
    	ContactsManager.getInstance().setContactCapabilitiesTimestamp(contact, System.currentTimeMillis());
	...
	private void handle200OK(SipTransactionContext ctx) {
    	...
    	if (capabilities.isImSessionSupported()) {
			...
			if(capabilities.isSipJoynOffline()){
				if (logger.isActivated()) {
            		logger.info("receiveCapabilityRequest Options Offline");
        		}
				...
				if (logger.isActivated()) {
            		logger.info("receiveCapabilityRequest Options Online");
        		}
    	 ...
    	 */
        if (logger.isActivated()) {
            logger.info("handle200OK setRcseContact contact: " + contact + " "
	...
	private void handleError(CapabilityError error) {
		...
		if (logger.isActivated()){
            logger.info("handleError Status:" + info.getRcsStatus());
			
			if(info.getCapabilities().isImSessionSupported()){
				logger.info("handleError Rcse true");
			}else{
				logger.info("handleError Rcse false");
			}
         }    	
