  ...
  if(opcode == 0){
	  ...
	  if (chipid >= MTK_6589_SUPPORT) {
	      if (l_clkIdx_pu != 0xF && l_cmdIdx_pu != 0xF && l_datIdx_pu != 0xF)
	  	  {
	  	      XLOGD("success: clk_pu=%d,  clk_pd = %d, cmd_pu=%d,  cmd_pd = %d, dat_pu=%d,  dat_pd = %d, rst_pu = %d, rst_pd = %d, ds_pu = %d, ds_pd = %d\r\n",
...
(JNIEnv * env, jobject obj, jint hostNum, jint clkPU, jint clkPD, jint cmdPU, jint cmdPD, jint datPU, jint datPD, jint rstPU, jint rstPD, jint dsPU, jint dsPD, jint hopBit , jint hopTime,jint opcode)
{
