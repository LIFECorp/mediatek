...
public class VideoFavoritesRootView extends RelativeLayout implements IMtkWidget,
    ...
    private void refreshAdapterCache() {
        AbsListView grid = findFirstAbsListView();
        if (grid != null) {
            BaseAdapter adapter = (BaseAdapter) grid.getAdapter();
            if (adapter != null) {
                Xlog.v(TAG, "notify gridViewAdapter");
    ...
    protected void onDetachedFromWindow() {
        ...
        stopAllVideoView();
        // Force adapter to refresh cache, so gridView won't use old thumbnail
        refreshAdapterCache();
        setState(DETACHED);
