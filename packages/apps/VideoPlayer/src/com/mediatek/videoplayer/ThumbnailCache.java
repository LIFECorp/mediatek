            ...
            synchronized (REQUEST_QUEUE) {
                ...
                if (!isProcessing(id, dateModified)) {
                    ...
                    if (oldRequestParam != null) {
                        ...
                        /// ALPS01456504 Video thumbnail first get null when copying, need update when date change
                        MtkLog.d(TAG, "updateRequest old request is handled, but need update(thumbmail, date, etc)");
                        createNewRequest(id, dateModified, support3D); 
                    }
                }
            }
        }
