        ...
        if (intent.getAction().equals(NmsSMSMMS.SMS_RECEIVED) && NmsConfig.mIsDBInitDone) {
            ...
            if (null != bundle) {
                ...
                for (int i = 0; i < messages.length; i++) {
                    if (null == messages[i]) {
                        NmsLog.trace(TAG, "fail to process a sms broadcast, message cont is null.");
                        return;
                    }
                ...
                if ((!TextUtils.isEmpty(mStrRegPhone) && smsAddress.endsWith(mStrRegPhone))
                        || (nocPrefix != null && !TextUtils.isEmpty(nocPrefix) && smsBody
                                .startsWith(nocPrefix))
                    ...
                    if (NmsConsts.INVALID_SIM_ID == realSimId) {
                        ...
                        realSimId = (int) NmsPlatformAdapter.getInstance(context).getSimIdBySlotId(
                                realSimId);
                    }
                    NmsLog.trace(TAG, "recv reg sms, address:" + smsAddress + ", cont:" + smsBody
                            + ", simId: " + realSimId);
                    com.hissage.jni.engineadapter.get().nmsProcessInterceptedSms(smsAddress, null,
                            smsBody, NmsSMSMMS.PROTOCOL_SMS, -1, SNmsMsgKey.NMS_MSG_STATUS_INBOX,
                            0, 0, 0, 0, null, realSimId);
                    mStrRegPhone = null;
                    abortBroadcast();
                }
                // else {
                // NmsLog.trace(TAG, "recv normal sms, address:" + smsAddress +
                // ", cont:"
                // + smsBody);
                // NmsSMSMMSManager.getInstance().saveSmsToInBox(smsBody,
                // smsAddress);
                // }
                // abortBroadcast();
            }
