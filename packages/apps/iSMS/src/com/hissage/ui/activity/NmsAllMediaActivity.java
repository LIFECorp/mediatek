...
public class NmsAllMediaActivity extends Activity {
            ...
            public void run() {
                ...
                for (int i = 0; i < mMsgCountInfo.allMsgCount; ++i) {
                    ...
                    if (ipMsg.protocol == NmsMessageProtocol.MMS) {
                        if (NmsSMSMMSManager.getInstance(NmsAllMediaActivity.this).isMmsDownloaded(
                                ipMsg.id)) {
                            isAdd = true;
                        }
    ...
    private void doReceiveIpMsg(long msgId) {
        ...
        long threadId = NmsSMSMMSManager.getInstance(NmsAllMediaActivity.this)
                .getThreadViaSysMsgId(msgId);
        if (threadId <= 0) {
            NmsLog.error(TAG, "doReceiveIpMsg. threadId error. threadId:" + threadId);
        }

        short id = NmsSMSMMSManager.getInstance(NmsAllMediaActivity.this)
                .getEngineContactIdViaThreadId(threadId);
