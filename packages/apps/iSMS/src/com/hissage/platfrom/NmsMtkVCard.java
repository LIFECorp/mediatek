...
public class NmsMtkVCard extends NmsPlatformBase {
    ...
    private String nmsGetVcfViaMTKPlatform(Context context, String contactsId) {
        if (null == context || TextUtils.isEmpty(contactsId)) {
            NmsLog.error(HissageTag.vcard, "get vcf file error, param: " + context
        ...
        try {
            ...
            try {
                ...
                if (!composer.init(Contacts._ID + " IN (" + contactsId + " )", null)) {
    ...
    public String nmsGetVcfViaSysContactId(Context context, long[] contactsId) {
        if (null == context || contactsId == null || contactsId.length <= 0) {
        ...
        if (null != mSystemVCardComposerClass && NMS_INTEGRATION_MODE == mPlatfromMode) {
            try {

                String path = NmsCommonUtils.getCachePath(context) + System.currentTimeMillis()
                        + "_" + contactsId.length + ".vcf";
                ...
                Constructor constructor = mSystemVCardComposerClass.getConstructor(param);

                Object instance = constructor.newInstance(objs);

                Method methodInit = mSystemVCardComposerClass.getMethod("init", String.class,
                        String[].class);
                Method methodIsAfterLast = mSystemVCardComposerClass.getMethod("isAfterLast");

                Method methodCreateOneEntry = mSystemVCardComposerClass.getMethod("createOneEntry");

                Method methodTerminate = mSystemVCardComposerClass.getMethod("terminate");

                if (!(Boolean) (methodInit.invoke(instance, Contacts._ID + " IN (" + contactStr
                        + ")", null))) {
                    throw new Exception("can not get vcf from MTK FW.");
                }

                while (!(Boolean) (methodIsAfterLast.invoke(instance))) {
                    writer.write((String) (methodCreateOneEntry.invoke((instance))));
                }

                methodTerminate.invoke(instance);
                writer.close();

                NmsLog.trace(TAG, "NmsMtkVCard get on contact vcf from mtk FW, contactsId:"
                        + contactStr);
