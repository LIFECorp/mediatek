...
public final class NmsStatistics extends SQLiteOpenHelper {
    ...
    public static int KEY_WAKEUP_ACTIVATE_PROMPT = 25;
    public static int KEY_WAKEUP_ACTIVATE_TRY = 26;
    public static int KEY_WAKEUP_ACTIVATE_OK = 27;

    private static final String DB_NAME = "nms_statistics.db";
    private static final String DB_TABLE_NAME = "statistics";
    private static final int DB_VERSION = 1;
    private static final int DB_LAZY_UPDATE_COUNT = 26;
    private static final int DB_LAZY_UPDATE_TIME = 5 * 60;

    private static final String LOG_TAG = "statistics";

    private static final String COLUMN_ARRAY[] = { "emo_activate_prompt", "emo_activate_try",
            "emo_activate_ok", "media_activate_prompt", "media_activate_try", "media_activate_ok",
            ...
            "tips_activate_prompt", "tips_activate_try", "tips_activate_ok", "sms_activate_prompt",
            "sms_activate_try", "sms_activate_ok", "wakeup_activate_prompt", "wakeup_activate_try",
            "wakeup_activate_ok" };
            ...
            for (String updateColumn : lazyUpdates) {
                db.execSQL(String
                        .format("UPDATE OR IGNORE %s SET %s=(SELECT %s FROM %s WHERE date='%s')+1 WHERE date='%s';",
                                DB_TABLE_NAME, updateColumn, updateColumn, DB_TABLE_NAME, curDate,
                                curDate));
            }
    ...
    private JSONObject doToJasonData() {
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                for (int i = 0; i < COLUMN_ARRAY.length; i++) {
                    dateObject.put(COLUMN_ARRAY[i], cursor.getInt(i + 1)); /*
                                                                            * +
                                                                            * 1
                                                                            * because
                                                                            * the
    ...
    /*
     * I insert all the public functions code into a try-catch block Yeah, it
     * may violate some JAVA criterion, and it's ugly in someone's option. but
     * it's safe, for an statistics module should never make the host app crash,
     * even it's NOT work.
     */
    ...
    public static void incKeyVal(int keyIndex) {
        ...
        try {
            ...
            if (keyIndex < 0 || keyIndex >= COLUMN_ARRAY.length) {
                NmsLog.error(LOG_TAG,
                        String.format("fatal error to inc keyIndex: %d is invalid", keyIndex));
                return;
            }
