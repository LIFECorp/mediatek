...
public class NmsQueue extends Thread {
    ...
    private void notifyDbFullStatus(NmsMessage msg) {
        ...
        if (SNmsMsgType.NMS_UI_MSG_DB_FULL == msgId)
            intent.putExtra(NmsIpMessageConsts.NmsStoreStatus.NMS_STORE_STATUS,
                    NmsIpMessageConsts.NmsStoreStatus.NMS_STORE_FULL);
        else
            intent.putExtra(NmsIpMessageConsts.NmsStoreStatus.NMS_STORE_STATUS,
                    NmsIpMessageConsts.NmsStoreStatus.NMS_STORE_ERR);
    ...
    private void refreshContact(NmsMessage msg) {
        ...
        intent.putExtra(NmsRefreshContactList.NMS_REFRESH_CONTACTS_LIST,
                SNmsMsgType.NMS_UI_MSG_CONTACT_REFRESH);
    ...
    private void notifyNocTime(NmsMessage msg) {
        NmsLog.trace(TAG, "get noc time msg");
        Intent intent = new Intent();
        intent.setAction(NmsConsts.NmsIntentStrId.NMS_REG_SEND_NOC_TIME);
        intent.putExtra(NmsConsts.NmsIntentStrId.NMS_REG_SEND_NOC_TIME, msg.getMsgDataLen());
        NmsService.getInstance().sendBroadcast(intent);
    }

    private void notifySelfTime(NmsMessage msg) {
        NmsLog.trace(TAG, "get self time msg");
        Intent intent = new Intent();
        intent.setAction(NmsConsts.NmsIntentStrId.NMS_REG_SEND_SELF_TIME);
        intent.putExtra(NmsConsts.NmsIntentStrId.NMS_REG_SEND_SELF_TIME, msg.getMsgDataLen());
        NmsService.getInstance().sendBroadcast(intent);
    }
    ...
    private void dispatchMessage(NmsMessage msg) {
        ...
        case SNmsMsgType.NMS_UI_MSG_REG_SMS_NOC_TIME:
            notifyNocTime(msg);
            break;

        case SNmsMsgType.NMS_UI_MSG_REG_SMS_SELF_TIME:
            notifySelfTime(msg);
            break;
