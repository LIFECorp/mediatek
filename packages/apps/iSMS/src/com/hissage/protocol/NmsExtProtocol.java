...
public class NmsExtProtocol {
    ...
    static private boolean doSendStatus(String imsi, String number, int clientSessionId,
            int serverSessionId) {
        ...
        try {
            ...
            listPair.add(new BasicNameValuePair("version", NmsService.getInstance().getString(
                    R.string.STR_NMS_VERSION_ID)));

            HttpPost httpPost = new HttpPost(NmsService.getInstance().getString(
                    R.string.STR_NMS_OFFLINE_URL));
            ...
            NmsLog.trace(TAG,
                    "get result: " + EntityUtils.toString(httpResp.getEntity(), HTTP.UTF_8));
    ...
    static public void sendOfflineStatusToServer(final String imsi, final String number,
            final int clientSessionId, final int serverSessionId) {

        try {
            NmsLog.trace(
                    TAG,
                    String.format(
                            "try to send status imsi: %s, number:%s, clientSessionId: %d, serverSessionId: %d, to server",
                            imsi, number, clientSessionId, serverSessionId));
