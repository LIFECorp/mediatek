...
static int t_u3auto_loopback_scan(int argc, char** argv){
	...
	int hs_isoc_maxp[HS_ISOC_MAXP_SIZE] = {6, /*8,32,*/128,/*512,*/1024};
	...
	int fs_isoc_maxp[FS_ISOC_MAXP_SIZE] = {6,/*8,32,*/128,1023};
						...
						for(burst_index = 0; burst_index < burst_arr_size; burst_index++){
							...
							for(start_add = min_start_add; start_add <= max_start_add; start_add++){
								...
								while(cur_sec != NULL){
                                    ...
                                    if((maxp % 4 == 2) && (transfer_type == EPATT_ISO || transfer_type == EPATT_INT)){
                                            printk( "             SKIP!!, HW BUG, maxp(%d) trans_type(%d)\n", maxp, transfer_type);
                                            cur_sec = find_next_num(cur_sec);
                                            continue;
                                    }
...
static int t_power_u2_swlpm(int argc, char** argv){
	...
	if(expected_L1S == 0){
        ...
        /* if expected_L1S != 0, go into L1 state */
...
static int t_hub_remotewakeup_dev(int argc, char** argv){
	...
	udev = dev_list[dev_num-1];
	g_slot_id = udev->slot_id;
	return dev_remotewakeup(length);
}
...
extern void usb_phy_recover();
#include <mach/mt_gpio.h>
#endif

static int dbg_u3PHY_init(int argc, char**argv){
    #if 0//CONFIG_PROJECT_PHY
    usb_phy_recover();
    #else
    u3phy_init();

    /* EVB power on sequence */
