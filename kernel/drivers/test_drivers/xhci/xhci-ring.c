	...
	struct xhci_virt_ep *ep, int *status, bool skip)
{
	...
	if (trb_comp_code == COMP_STOP_INVAL ||
		...
		if (trb_comp_code == COMP_STALL) {
			...
			xhci_dbg(xhci, "inc dequeue of ring: dequeue(0x%x), last_trb(0x%x)\n", ep_ring->dequeue, td->last_trb);
	...
	struct xhci_virt_ep *ep, int *status)
{
    ...
    {
        int ret = finish_td(xhci, td, event_trb, event, ep, status, false);
        return ret;
	//return finish_td(xhci, td, event_trb, event, ep, status, false);
    }
}
		...
		struct xhci_event_cmd *event)
{
	...
	xhci_dbg(xhci, "CMD(%d), Complete Code(%d), slot_id(%d)\n",
	...
	case TRB_TYPE(TRB_ADDR_DEV):
		if(GET_COMP_CODE(event->status) == COMP_SUCCESS){
			...
			xhci_dbg(xhci, "Address Device Aborted\n");
			g_cmd_status = CMD_DONE;
		}
		else{
			g_cmd_status = CMD_FAIL;
...
//	temp = xhci_readl(xhci, addr);
	...
	xhci_dbg(xhci, "port_id(%d), state(0x%x), neutral(0x%x), status(0x%x)\n", port_id, temp, temp1, status);
#if 0
...
//			port->port_id = 0;
	...
	if((port_status & (USB_PORT_STAT_C_RESET << 16)) && (port_status & (USB_PORT_STAT_CONNECTION)) && !(port_status & USB_PORT_STAT_SUPER_SPEED)){
		...
		if(!(port_status & USB_PORT_STAT_RESET)){
			...
			port->port_status = ENABLED;
            xhci_dbg(xhci, "port_reenabled(%d)\n", port->port_reenabled);
			if(port->port_reenabled == 1){
				port->port_reenabled = 2;
			}
			if(g_device_reconnect == 1)
		...
		struct xhci_transfer_event *event)
{
    ...
    xhci_dbg(xhci, "deg_seg_dma(0x%x)trb(0x%x), dequeue(0x%x), last_trb(0x%x), event_seg_dma(0x%x), event_dma(0x%x)trb(0x%x)\n",
            ep_ring->deq_seg->dma, ep_ring->deq_seg->trbs, ep_ring->dequeue, td->last_trb, event_seg->dma, event_dma, event_trb);
...
int xhci_handle_event(struct xhci_hcd *xhci){
    ...
    case TRB_TYPE(TRB_HC_EVENT):
        xhci_dbg(xhci, "Host Contoller Event, comp code (0x%x)\n", GET_COMP_CODE(event->generic.field[2]));
        break;
	default:
		break;
#if 0
