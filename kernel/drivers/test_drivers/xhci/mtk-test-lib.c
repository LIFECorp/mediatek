...
, dma_addr_t dma_mapping, struct usb_device *usbdev){
	...
	if(usb_endpoint_xfer_bulk(&ep->desc) && usb_endpoint_dir_out(&ep->desc)){
		...
		for(j=0; j<iso_num_packets; j++){
			...
			xhci_dbg(xhci, "out iso frame offset(%d) length(%d)\n",
                        urb->iso_frame_desc[j].offset, urb->iso_frame_desc[j].length);
		}
	}
		...
		for(j=0; j<iso_num_packets; j++){
			...
			xhci_dbg(xhci, "in iso frame offset(%d) length(%d)\n",
                        urb->iso_frame_desc[j].offset, urb->iso_frame_desc[j].length);
		}
	}
	urb->transfer_dma = mapping;
...
//		printk(KERN_ERR "[Debug] max_esit_payload: %d\n", max_esit_payload);
    ...
    xhci_dbg(xhci, "Tx stop\n");
    xhci_dbg(xhci, "Rx start\n");
	//Rx
...
//	g_port_reset = false;
	enablePortClockPower(port_index,0x2);
