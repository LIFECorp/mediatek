...
#elif defined(MT6630)
    pcFwName = "/data/misc/"CFG_FW_FILENAME"_MT6630";
...
#if DBG
        //DBGLOG_MEM8(RX, TRACE, prSkb->data, prSkb->len);
#endif
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 26)
        if (GLUE_GET_REF_CNT(prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex][u2QueueIdx]) >= CFG_TX_STOP_NETIF_PER_QUEUE_THRESHOLD) {
    ...
    )
{
    ...
    if(fgIsValidDevice == TRUE) {
        ...
        if (netif_subqueue_stopped(prDev, prSkb) &&
                prGlueInfo->ai4TxPendingFrameNumPerQueue[ucBssIndex][u2QueueIdx] <= CFG_TX_START_NETIF_PER_QUEUE_THRESHOLD) {
    ...
    )
{
    ...
    *pfgIsVlanExists = FALSE;
    ...
    /* No need to indicate PAL packet (?) */
#if 0    
    if(*pfgIs802_3) {
...
int rx_thread(void *data)
{
    ...
    while (TRUE) {
        ...
        if (test_and_clear_bit(GLUE_FLAG_RX_TO_OS_BIT, &prGlueInfo->u4Flag)) {
            ...
            while(prQueueEntry) {
                kalRxIndicatePktsMthread(prGlueInfo, (PVOID)GLUE_GET_PKT_DESCRIPTOR(prQueueEntry));
                QUEUE_REMOVE_HEAD(prTempRxQue, prQueueEntry, P_QUE_ENTRY_T);
            }
