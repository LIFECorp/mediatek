                ...
                if (authProcessRxDeauthFrame(prSwRfb,
                    ...
                    if (prAisSpecBssInfo->fgMgmtProtection && HAL_RX_STATUS_GET_SEC_MODE(prSwRfb->prRxStatus) != CIPHER_SUITE_BIP) {
                        ...
                        if (!IS_BMCAST_MAC_ADDR(prDeauthFrame->aucDestAddr) && 
                            ...
                            #if DBG
                            DBGLOG(RSN, TRACE, ("QM RX MGT: rsnStartSaQuery\n"));
                            #endif
                            ...
                            #if DBG
                            DBGLOG(RSN, TRACE, ("RXM: Drop unprotected Mgmt frame  \n"));
                            DBGLOG(RSN, TRACE, ("RXM: (MAC RX Done) RX (u2StatusFlag=0x%x) (ucKIdxSecMode=0x%x) (ucWlanIdx=0x%x)\n", \
        ...
        if (IS_STA_IN_AIS(prStaRec)) {
            ...
            if (prStaRec->ucStaState > STA_STATE_1) {
                ...
                if (assocProcessRxDisassocFrame(prAdapter,
                    ...
                    DBGLOG(RSN, TRACE, ("QM RX MGT: Disassoc frame, P=%d Sec=%d CM=%d BC=%d fc=%02x\n",
