    ...
    )
{
    ...
    if(prMsduInfo) {
        ...
        DBGLOG(TX, TRACE, ("Cannot get Target Msdu WIDX:PID[%u:%u] from Pending Q\n", ucWlanIndex, ucPID));
    }
...
#if 0 && CFG_ENABLE_BT_OVER_WIFI /* disabled for BOW to finish ucBssIndex migration */
    ...
    )
{
    ...
    kalPrint("\n[%s][wifi][Firmware] Assert at \"%s\" #%ld\n\n",
        NIC_NAME,
        aucAssertFile,
        line);
    ...
    case NETWORK_TYPE_AIS:
        if(GET_BSS_INFO_BY_INDEX(prAdapter, ucBssIndex)->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
            ...
            if(prAdapter->fgIsLinkQualityValid == FALSE
                ...
                nicUpdateRSSI(prAdapter, ucBssIndex, prEventLinkQuality->cRssi, prEventLinkQuality->cLinkQuality);
            }

            if(prAdapter->fgIsLinkRateValid == FALSE
                    || (kalGetTimeTick() - prAdapter->rLinkRateUpdateTime) > CFG_LINK_QUALITY_VALID_PERIOD) {
                nicUpdateLinkSpeed(prAdapter, ucBssIndex, prEventLinkQuality->u2LinkSpeed);
            }
        }
        break;
