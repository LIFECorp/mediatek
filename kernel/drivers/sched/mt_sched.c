...
static long __mt_sched_setaffinity(pid_t pid, const struct cpumask *in_mask)
{
	...
	if (!p) {
		rcu_read_unlock();
		put_online_cpus();
		pr_err("MT_SCHED: setaffinity find process %d fail\n", pid);
		return -ESRCH;
	}
	...
	if (!alloc_cpumask_var(&new_mask, GFP_KERNEL)) {
		retval = -ENOMEM;
		pr_err("MT_SCHED: setaffinity allo_cpumask_var for new_mask fail\n");
	...
	if (retval) {
		pr_err("MT_SCHED: setaffinity security_task_setscheduler fail, status: %d\n",
		       retval);
		goto out_unlock;
	}
...
again:
	...
	if (retval)
		pr_err("MT_SCHED: set_cpus_allowed_ptr status %d\n", retval);
...
out_put_task:
	...
	if (retval)
		pr_err("MT_SCHED: setaffinity status %d\n", retval);
...
static long __mt_sched_getaffinity(pid_t pid, struct cpumask *mask, struct cpumask *mt_mask)
{
	...
	p = find_process_by_pid(pid);
	if (!p) {
		pr_err("MT_SCHED: getaffinity find process %d fail\n", pid);
		goto out_unlock;
	}

	retval = security_task_getscheduler(p);
	if (retval) {
		pr_err("MT_SCHED: getaffinity security_task_getscheduler fail, status: %d\n",
		       retval);
		goto out_unlock;
	}
...
out_unlock:
	...
	if (retval)
		pr_err("MT_SCHED: getaffinity status %d\n", retval);
...
static long __mt_sched_exitaffinity(pid_t pid)
{
	...
	spin_unlock_irqrestore(&mt_sched_spinlock, irq_flags);
	if (!find) {
		pr_err("MT_SCHED: eixtaffinity find process %d fail.\n", pid);
		return -ESRCH;
	}
	return 0;
}
...
static int __init sched_ioctl_init(void)
{
	...
	if (alloc_chrdev_region(&sched_dev_num, 0, 1, SCHED_DEV_NAME)) {
		pr_err("MT_SCHED: Device major number allocation failed\n");
		return -EAGAIN;
	}

	sched_ioctl_cdev = cdev_alloc();
	if (NULL == sched_ioctl_cdev) {
		pr_err("MT_SCHED: cdev_alloc failed\n");
		ret = -1;
		goto out_err2;
	}
	...
	if (ret) {
		pr_err("MT_SCHED: Char device add failed\n");
		goto out_err2;
	}
