...
static int ion_system_heap_shrink(struct shrinker *shrinker,
		...
		//in LCA project, we want to shrink ion pool quickly 
		sc->nr_to_scan = max(sc->nr_to_scan, 8192);
...
void ion_mm_heap_memory_detail(void) {
    ...
    for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
        ...
        if (client->task) {
            ...
            ION_PRINT_LOG_OR_SEQ(NULL, "%16.s %16u %16u 0x%p\n", task_comm,
...
struct ion_heap *ion_mm_heap_create(struct ion_platform_heap *unused)
{
    ...
    heap->heap.shrinker.seeks = 1;
    heap->heap.shrinker.batch = 16;
