...
void VideoQualityController::adjustForTemperature() {
	ALOGD("mFPSAdjustedForTemp(%d) +", mFPSAdjustedForTemp);
		...
		char path[20];
		char vrt_status;
		snprintf(path, 20, "/proc/driver/cl_vrt");
		FILE *fp = fopen(path, "rb");

		if (!fp) {
			ALOGE("- Open file /proc/driver/cl_vrt fail...");
			return;
		}

		if (1 != (int32_t)fread(&vrt_status, sizeof(signed char), 1, fp)){
			ALOGE("- Read file /proc/driver/cl_vrt fail...");
			fclose(fp);
			return;
		}

		ALOGD("read cl_vrt: %d", vrt_status-'0');
		fclose(fp);
		
		sp<OMXCodec> omx_enc =reinterpret_cast<OMXCodec *>(mSource.get()); 
		
		if(vrt_status=='1' && !mFPSAdjustedForTemp) {
			if(mFPSAdjustedForMem) {
				ALOGI("Fps has been adjusted for memory, ignore for temperature");
				return;
			}		

			ALOGD("slow framerate to %d", (int32_t)(mVideoInitFPS*0.8));
			
			status_t err = reinterpret_cast<CameraSource *>((omx_enc->getSource()).get())
				->setFrameRate(mVideoInitFPS*0.8);
			if (err != OK) {
				ALOGE("camera set framerate error(%d)",  err);
			}	
			
			err = omx_enc->vEncSetFrameRate(mVideoInitFPS*0.8);	
			if (err != OK) {
				ALOGE("encoder set framerate error(%d)", err);
			}	
			mFPSAdjustedForTemp = true;
		}
		
		if(vrt_status=='0' && mFPSAdjustedForTemp) {
			
			ALOGD("recover framerate to %d", mVideoInitFPS);
			status_t err = reinterpret_cast<CameraSource *>((omx_enc->getSource()).get())
				->setFrameRate(mVideoInitFPS);
			if (err != OK) {
				ALOGE("camera set framerate error(%d)", err);
			}	
			
			err = omx_enc->vEncSetFrameRate(mVideoInitFPS);	
			if (err != OK) {
				ALOGE("encoder set framerate error(%d)", err);
			}	
			mFPSAdjustedForTemp = false;
