...
RESET_SEEK_TIME:
    ...
    {
       if(mExtractor->getNALParserType() != SIZE_NAL_TYPE) {
        retVal = mExtractor->GetNextMediaFrame(out, bIsKeyFrame,mType,&mSeeking,mTrackIndex);
    ...
    if ((mType == ASF_VIDEO) && (!mWantsNALFragments) && ((!mIsMJPEG))) {
		if(mIsAVC && (SIZE_NAL_TYPE == mExtractor->getNALParserType())) {
			bool bIsKeyFrame = false;
			...
			ALOGI ("ASFSource::read()Video Type = AVC,mWantsNALFragments=false,SIZE_NAL_TYPE ");
            ...
            while(mBuffer->range_length() != 0 ) {
					...
					CHECK(mBuffer != NULL);
					mBuffer->set_range(
										mBuffer->range_offset() + nalSizeLength,
					...
					tempBuffer->set_range(tempBuffer->range_offset() +nalLength + 3,tempBuffer->size() - tempBuffer->range_offset() - nalLength - nalSizeLength);
			...
			if(mBuffer->range_length() == 0) {
				...
				memcpy(reBuffer->data() + reBuffer->range_offset(),tempBuffer->data() + tempBuffer->range_offset(),tempBuffer->range_length());
    ...
    if (mType == ASF_VIDEO && mIsAVC && mWantsNALFragments)
    {
		...
		if (newBuffer && (!isSeek)) {
			...
			if (ASF_END_OF_FILE == retVal) {
				...
				//post-process after find a frame , for NALFragment AVC video
					//if(SIZE_NAL_TYPE != mExtractor->getNALParserType()){
				retVal = ReadFramePostProcess(&mBuffer);
				if (retVal != ASF_SUCCESS) {
					return ERROR_END_OF_STREAM;
				}
        ...
        }else if(START_CODE_TYPE == mExtractor->getNALParserType()){						
...
uint32_t ASFExtractor::ParserAVCFrameType(uint8_t* data, int size)
{
    ...
    while (start < end) {
        int i_nal_type=0;
		if(SIZE_NAL_TYPE == getNALParserType()){
			NALLength = parseNALSize((uint8_t *)start);
			if(0 == NALLength){
				return SKIPPED;
			}
			start += offsetStart + iNALSizeLength;
			offsetStart = offsetStart + NALLength;
		}else if (START_CODE_TYPE == getNALParserType()){
			offsetStart = nalStartAt(start, end - start, &prefixLen);
			if (offsetStart == -1 || start >= end){
				return SKIPPED;
			}			
			start += offsetStart + prefixLen;
		}
        i_nal_type = start[0] & 0x1f;
        if (5 == i_nal_type) {
            return IVOP;
        }
		//NAL_SLICE: i_nal_type >= 1 
		//NAL_SLICE_IDR: i_nal_type < 5
        if (i_nal_type >= 1  && i_nal_type < 5 ) {
            byte = start[1];
            break;
        }
    }
    ...
    else if(!strcasecmp(mime_present,MEDIA_MIMETYPE_VIDEO_AVC))
    {
		if(_config_size_extra_data != 0) {
			...
			if( true == isNALStartCodeType(csd)) {
			   ...
			   eNalParType = SIZE_NAL_TYPE;
			}
			ALOGI("eNalParType = %d",eNalParType);
			
			if(eNalParType == SIZE_NAL_TYPE ) {
				sizeLength = getLengthSizeMinusOne(csd) + 1;	
				ALOGI("sizeLength = %ld",sizeLength);
			}
...
status_t ASFExtractor::addAVCCodecSpecificData(const sp<MetaData>& mMeta) {
	...
	iSPSPos = realAVCStart(buffer->data(), size);//SPS position= ptr+AVCpos
	if(-1 == iSPSPos){
	...
	while (bufstart < bufend) { //find pps
		...
		if(true == IsFindPPS){
			if (bufstart > ((uint8_t*)buffer->data()+ iSPSPos)) {
				...
				buffer->setRange(iSPSPos, offsetStart);
			}else{
...
bool ASFExtractor::isNALStartCodeType(const sp<ABuffer> &buffer) {	
	...
	iPPSPos = realPPSStart(buffer->data(), buffer->size());
	if (-1 == iPPSPos) {
...
status_t ASFExtractor::asfMakeAVCCodecSpecificData(const sp<ABuffer> &buffer,const sp<MetaData>& mMeta){
	sp<MetaData> metaAVC;
	int32_t width = 0; 
	int32_t height = 0;
    const void *csd = NULL;
