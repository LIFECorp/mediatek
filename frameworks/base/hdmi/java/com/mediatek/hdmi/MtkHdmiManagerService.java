...
public final class MtkHdmiManagerService extends IMtkHdmiManager.Stub {
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        pw.println("HDMI mSupportedFeatures: " + mSupportedFeatures);
    ...
    @Override
    public boolean isFeatureSupported(int mask) {
        log(TAG, "isFeatureSupported: " + mask);
        return (mSupportedFeatures & mask) != 0;
    }

    private void getSupportedFeatures() {
        synchronized (mHdmiNative) {
            mSupportedFeatures = mHdmiNative.getSupportedFeatures();
            log(TAG, "getSupportedFeatures: 0x"
                    + Integer.toHexString(mSupportedFeatures));
            log(TAG, "FEATURE_SCALE_ADJUST: "
                    + ((IHDMINative.FEATURE_SCALE_ADJUST & mSupportedFeatures) != 0));
        }
    }
