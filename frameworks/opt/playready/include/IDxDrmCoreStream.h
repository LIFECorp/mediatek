...
public:
	...
	virtual EDxDrmContentType GetDRMContentType()	{return DX_CONTENT_TYPE_DRM_FILE_AND_STREAM;	}
	...
	virtual EDxDrmStatus ProcessPayloadEx(
		...
		DxUint32 decryptedPayloadOffset) = 0;
...
//! - DX_SUCCESS - On Success
//! - DX_ERROR_BAD_ARGUMENTS - If invalid arguments were passed.
	virtual EDxDrmStatus CopyToBuffer(
		void* srcBuffer,
		DxUint32 srcBufferSize,
		void* dstBuffer,
		DxUint32* dstBufferSize,
		DxBool isSecureBuffer,
		DxUint32 dstOffset ) = 0;
	...
	//! - DX_NOT_AVAILABLE - Track or AUFormatBox was not found.
	//! - DX_ERROR_INVALID_FORMAT - Packet data has invalid format	
	virtual EDxDrmStatus ProcessMultiplePayloads(
		 ...
		 DxUint32* decryptedPayloadSize) 
	{
		...
		(void)decSettings;
		(void)payloadData;
		(void)payloadDataSize;
		(void)decryptedPayloadData;
		(void)decryptedPayloadSize; 
