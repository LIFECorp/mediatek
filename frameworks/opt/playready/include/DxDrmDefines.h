...
// DX_DRM_NO_EXPORTS
#if defined(DX_DRM_NO_EXPORTS)
#define DRM_C_API
// not defined DX_DRM_NO_EXPORTS
// if Windows
...
#define DRM_C_API __attribute__((visibility("default")))
// if Other OS
#else
#define DRM_C_API
#endif
...
typedef enum EDxDrmStatus {
    ...
    DX_ERROR_RIGHTS_NOT_AVAILABLE,              //!< The rights the app has requested are not available in the license.
    DX_ERROR_LICENSE_EXPIRED,                   //!< The license has expired either by depleting a play count or via an end time.
...
typedef enum EDxStoreAsset {
	...
	DX_STORE_PLAYREADY_PROVISION_PACKAGE_DIGEST,        // Hash digest for provisioning package integrity (non-secure)
...
typedef enum EDxAsset {
    DX_ASSET_OMA_V1_RO						= 0x00000001,
    DX_ASSET_OMA_V2_RO						= 0x00000002,
    DX_ASSET_OMA_V2_RI_CONTEXT				= 0x00000004,
    DX_ASSET_OMA_V2_DOMAIN_CONTEXT			= 0x00000008,
    DX_ASSET_OMA_V2_WHITE_LIST				= 0x00000010,
	DX_ASSET_OMA_V2_METERING				= 0x00000020,
	DX_ASSET_OMA_V2_METERING_CONSENT		= 0x00000040,
    DX_ASSET_OMA_V2_PROVISIONING			= 0x00000080,
    DX_ASSET_WM_LICENSE						= 0x00000100,
    DX_ASSET_WM_PROVISIONING				= 0x00000200,
    DX_ASSET_INTERNAL_CACHE					= 0x00000400,
	DX_ASSET_SECURE_CLOCK					= 0x00000800,
	DX_ASSET_HTTP_AUTHENTICATION			= 0x00001000,
	DX_ASSET_PLAYREADY_PROVISIONING			= 0x00002000,
	DX_ASSET_PLAYREADY_STORE				= 0x00004000,
	DX_ASSET_PLAYREADY_KEYFILE				= 0x00008000,
	DX_ASSET_PLAYREADY_PROVISIONING_PACKAGE	= 0x00010000,
    DX_ASSET_ALL							= 0xFFFFFFFF
} EDxAsset;
...
typedef struct __tagDxDecryptionSubSample
{
    DxUint32 dwClearDataSize;           /*!< Size of the data to be copied from the encBuffer to the output secure buffer (decryptedPayloadData) */
    DxUint32 dwEncryptedDataSize;       /*!< Size of the data to be decrypted into the decryptedPayloadData */
} DxDecryptionSubSample;

//! Holds additional data required for decryption 
//! IMPORTANT: When dwSubSamplesNum is bigger than '1', then a big-enough buffer should be allocated so DxDecryptionSettings is located in a contiguous memory.
typedef struct __tagDxDecryptionSettings
{
    DxUint64 qwInitializationVector;    /*!< 8 bytes of the constant part of the initialization vector (IV, from the PIFF file) */
    DxUint32 dwOutBufferOffset;         /*!< Offset into the output buffer to which decryption will be performed */
    DxUint32 dwMediaOffset ;            /*!< Normally Will be zero. Will be non zero in case NAL units in the same frame will be passed in different calls. */
    DxUint32 dwSubSamplesNum;           /*!< Number of DxSubSample structures in the DxDecryptionSettings.sampleArr */
	DxUint32 dwFlags;					/*!< First bit is used for passing Miracast connection status (1 = connected, 0 - not). Can be used for other features if needed */
    DxBool   bIsSecureBuffer;           /*!< when TRUE, decryptedPayloadData is required to be a secure buffer */
    DxDecryptionSubSample sampleArr[1]; /*!< IMPORTANT: This is only a pointer to the beginning of the array. The actual size must be allocated according to dwSubSamplesNum. */
}DxDecryptionSettings;

//! Gets the required size in bytes of the DxDecryptionSettings.sampleArr, based on _dwSubSamplesNum parameter.
#define DX_GET_DECRYPTION_SETTINGS_SAMPLE_ARR_SIZE(_dwSubSamplesNum) \
    ((_dwSubSamplesNum) * sizeof(DxDecryptionSubSample))

//! Gets the required size in bytes of the DxDecryptionSettings struct, based on _dwSubSamplesNum parameter.
#define DX_GET_DECRYPTION_SETTINGS_ACTUAL_LEN(_dwSubSamplesNum) \
    (sizeof(DxDecryptionSettings) + DX_GET_DECRYPTION_SETTINGS_SAMPLE_ARR_SIZE((_dwSubSamplesNum) - (sizeof(((DxDecryptionSettings *)0)->sampleArr) / sizeof(DxDecryptionSubSample))))

typedef struct  
{
	DxUint32 m_CompressedDigitalVideo;
	DxUint32 m_UncompressedDigitalVideo;
	DxUint32 m_AnalogVideo;
	DxUint32 m_CompressedDigitalAudio;
	DxUint32 m_UncompressedDigitalAudio;
	DxBool	 m_MiracastEnablerExist;
	DxBool	 m_AirPlayEnablerExist;
	DxBool   m_DigitalAudioProtectionExist;
} DxOutputLevelData;

typedef struct  
{
	DxOutputLevelData m_OutputLevelData;
} DxOemContext;
