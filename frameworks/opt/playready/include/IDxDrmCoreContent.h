...
public:
	...
	//! returns the secure clock challenge for the content
	virtual  EDxDrmStatus GetSecureClockChallenge( DxChar* url, DxUint32* urlSize, void* challenge, DxUint32* challengeSize) {urlSize=urlSize; url=url; challengeSize=challengeSize; challenge=challenge ; return DX_ERROR_NOT_SUPPORTED;};
	
	//! process the secure clock response arrived from the server	
	virtual	 EDxDrmStatus ProcessSecureClockResponse( void* serverResponse, DxUint32 serverResponseSize){serverResponse = serverResponse; serverResponseSize = serverResponseSize; return DX_ERROR_NOT_SUPPORTED;};
