...
//! Gets a secure clock challenge for the file handle given. 
//! \param[in]		drmFile			A handle previously created by DxDrmClient_OpenDrmFile()
//! \param[out]		url				A pointer to the buffer to hold the Secure Clock url data. if not null the url data will be copied to this buffer.
//! \param[in, out] urlSize			A pointer to the size of the url buffer. this will hold the size of the secure clock url. May not be null.
...
//! \param[in, out] challengeSize	A pointer to the size of the challenge buffer. this will hold the size of the secure clock challenge. May not be null.
...
//! - DX_ERROR_BUFFER_IS_TOO_SMALL - buffer pointer is null or buffer size given is too small, in this case the needed size will be returned in the size parameter.
DX_FUNC DRM_C_API EDxDrmStatus DxDrmFile_GetSecureClockChallenge(HDxDrmFile drmFile,  DxChar* url, DxUint32* urlSize, void* challenge, DxUint32* challengeSize);
...
//! \return
//! - DX_SUCCESS - Operation completed successfully.
DX_FUNC DRM_C_API EDxDrmStatus DxDrmFile_ProcessSecureClockResponse(HDxDrmFile drmFile, void* serverResponse, DxUint32 serverResponseSize);
