...
//! \param[in]	   decryptedPayloadOffset offset in decrypted buffer where starting to write
//! \return
//! - DX_SUCCESS - On Success
//! - DX_ERROR_SECURE_STORAGE_IS_CORRUPTED - If secure storage is used and found to be corrupted.
//! - DX_ERROR_IPC_FAILURE - Communication with other DRM agent component could not be established.
...
DX_FUNC DRM_C_API EDxDrmStatus DxDrmStream_ProcessPayloadEx(
	...
	DxUint32 decryptedPayloadOffset );
...
//! Gets a secure clock challenge for the stream handle given. 
//! \param[in]		drmStream		A handle previously created by DxDrmClient_OpenDrmStream()
//! \param[out]		challenge		A pointer to the buffer to hold the Secure Clock url data. if not null the url data will be copied to this buffer.
//! \param[in, out] challengeSize	A pointer to the size of the url buffer. this will hold the size of the secure clock url. May not be null.
//! \param[out]		challenge		A pointer to the buffer to hold the Secure Clock challenge data. if not null the challenge data will be copied to this buffer.
//! \param[in, out] size			A pointer to the size of the challenge buffer. this will hold the size of the secure clock challenge. May not be null.
//! \note The return url holds the petition url. The client should request the actual time server url using the petition url,
//!     and request the secure time using the time server url.
//! - DX_ERROR_BAD_ARGUMENTS - If drmStream or size is null.
//! - DX_ERROR_BUFFER_IS_TOO_SMALL - buffer pointer is null or buffer size given is too small, in this case the needed size will be returned in the size parameter.
DX_FUNC DRM_C_API EDxDrmStatus DxDrmStream_GetSecureClockChallenge(HDxDrmStream drmStream,  DxChar* url, DxUint32* urlSize, void* challenge, DxUint32* challengeSize);

//! Process secure clock response.
//! \param[in] drmStream	     A handle previously created by DxDrmClient_OpenDrmStream()
//! \param[in]	serverResponse		Buffer that contains the server response.
//! \param[in]	serverResponseLen	serverResponse buffer length
//! \return
//! - DX_SUCCESS - Operation completed successfully.
DX_FUNC DRM_C_API EDxDrmStatus DxDrmStream_ProcessSecureClockResponse(HDxDrmStream drmStream, void* serverResponse, DxUint32 serverResponseSize);
...
//! The CopyToBuffer() function copies the src buffer into dst buffer in Trust Zone domain from given offset
//! \param[in]     drmStream	     A handle previously created by DxDrmClient_OpenDrmStream()
//! \param[in]	   srcBuffer       Points to the source buffer
//! \param[in]	   srcBufferSize   Size of the source buffer.
//! \param[out]	   dstBuffer       Points to the destination buffer
//! \param[out]	   dstBufferSize   On exit will hold the size of the destination data.
//! \param[in]	   secureBufferType     type of the dst buffer i.e pointer/handle
//! \param[in]	   dstOffset     offset in the dst buffer 
//! \return
//! - DX_SUCCESS - On Success
//! - DX_ERROR_BAD_ARGUMENTS - If invalid arguments were passed.
DX_FUNC DRM_C_API EDxDrmStatus DxDrmStream_CopyToBuffer(
	HDxDrmStream drmStream,
	void* srcBuffer,
	DxUint32 srcBufferSize,
	void* dstBuffer,
	DxUint32* dstBufferSize,
	DxBool isSecureBuffer,
	DxUint32 dstOffset );

//! The ProcessMultiplePayloads() function decrypt a buffer that contains both clear and encrypted data.
// the data is kept in "samples" of NAL's of the same frame, that share the same IV
//! \param[in]     drmStream	  A handle previously created by DxDrmClient_OpenDrmStream()
//! \param[in]	   decSettings    information needed for the all payloads such as IV, offset, flags. The allocated buffer ends with the samplesArr and therefore it is the size of (DxDecryptionSettings + (samplesArrCount * DxSubSample)) and not only size of DxDecryptionSettings.
//! \param[in]	   payloadData       Points to the encrypted buffer
//! \param[in]	   payloadDataSize   size of encrypted buffer
//! \param[out]	   decryptedPayloadData       Points to the destination decrypted buffer
//! \param[out]	   decryptedPayloadSize   On exit will hold the size of the destination data.
//! \return
//! - DX_SUCCESS - On Success
//! - DX_ERROR_SECURE_STORAGE_IS_CORRUPTED - If secure storage is used and found to be corrupted.
//! - DX_ERROR_IPC_FAILURE - Communication with other DRM agent component could not be established.
...
//! - DX_NOT_AVAILABLE - Track or AUFormatBox was not found.
//! - DX_ERROR_INVALID_FORMAT - Packet data has invalid format	
DX_FUNC DRM_C_API EDxDrmStatus DxDrmStream_ProcessMultiplePayloads(
	HDxDrmStream drmStream,
	DxDecryptionSettings* decSettings,
	void* payloadData,
	DxUint32 payloadDataSize,
	void* decryptedPayloadData,
	DxUint32* decryptedPayloadSize);
