...
public class NoteBookBackupComposer extends Composer {
    ...
    public final boolean init() {
        ...
        MyLogger.logD(CLASS_TAG, "init():" + result + ",count::" + (mCursor != null ? mCursor.getCount() : 0));
        return result;
    }
    ...
    public final boolean implementComposeOneEntity() {
        ...
        if (mCursor != null && !mCursor.isAfterLast()) {
            if (mXmlComposer != null) {
                try {
                    String title = mCursor.getString(mCursor.getColumnIndexOrThrow(COLUMN_NAME_TITLE));
                    String note = mCursor.getString(mCursor.getColumnIndexOrThrow(COLUMN_NAME_NOTE));
                    String created = mCursor.getString(mCursor.getColumnIndexOrThrow(COLUMN_NAME_CREATED));
                    String modified = mCursor.getString(mCursor.getColumnIndexOrThrow(COLUMN_NAME_MODIFIED));
                    String notegroup = mCursor.getString(mCursor.getColumnIndexOrThrow(COLUMN_NAME_GROUP));
                    ...
                    e.printStackTrace();
                    result = false;
                    MyLogger.logD(CLASS_TAG, "implementComposeOneEntity add xml error");
    ...
    public void onStart() {
        ...
        if (getCount() > 0) {
            ...
            if (mXmlComposer != null) {
                try {
                    mXmlComposer.startCompose(path);
                } catch (IOException e) {
                    e.printStackTrace();
                    MyLogger.logD(CLASS_TAG, "onStart xml create file error");
                    if (super.mReporter != null) {
                        super.mReporter.onErr(e);
                    }
                }
    ...
    public boolean onEnd() {
        ...
        if (mXmlComposer != null) {
            try {
                ...
                e.printStackTrace();
                MyLogger.logD(CLASS_TAG, "onEnd complate xml error");
                if (super.mReporter != null) {
                    super.mReporter.onErr(e);
                }
    ...
    /*
     * private void writeToFile(String inBuf) throws IOException { try {
     * if(outStream == null) { outStream = new
     * FileOutputStream(mParentFolderPath + File.separator +
     * ModulePath.FOLDER_NOTEBOOK + File.separator +
     * ModulePath.NOTEBOOK_XML,true); } byte[] buf = inBuf.getBytes();
     * outStream.write(buf, 0, buf.length); outStream.flush(); } catch
     * (IOException e) { if (super.mReporter != null) {
     * super.mReporter.onErr(e); } e.printStackTrace(); throw e; } }
     */
}
