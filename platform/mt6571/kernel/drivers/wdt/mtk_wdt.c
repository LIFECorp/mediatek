...
{  
	...
	tmp = MTK_WDT_EN_FAST_KEY;
	if(wdt_en == TRUE)
		tmp |= MTK_WDT_EN_FAST_ENABLE;

	DRV_WriteReg32(MTK_WDT_EN_FAST, tmp);
...
void mtk_wdt_restart(enum wd_restart_type type)
{
	...
	else if(type == WD_TYPE_NOLOCK) 
	{
				...
				spm_wdt_restart();
#else			
	      *(volatile u32 *)( MTK_WDT_RESTART_FAST) =MTK_WDT_RESTART_FAST_KEY ;
#endif
	}
	else
	{
...
void wdt_dump_reg(void)
{
	...
	printk("MTK_WDT_RESTART_FAST:0x%x\n", DRV_Reg32(MTK_WDT_RESTART_FAST));
...
int mtk_wdt_swsysret_config(int bit,int set_value)
{
	...
	spin_lock(&rgu_reg_operation_spinlock);
	wdt_sys_val = DRV_Reg32(MTK_WDT_FAST_SWSYSRST);
	printk("fwq2 before set wdt_sys_val =%x\n",wdt_sys_val);
    wdt_sys_val |= MTK_WDT_FAST_SWSYS_RST_KEY;
	switch(bit)
	{
	  case MTK_WDT_FAST_SWSYS_RST_MD_RST:
	  	   if(1==set_value)
	  	   {
	  	     wdt_sys_val |= MTK_WDT_FAST_SWSYS_RST_MD_RST;
	  	   }
		   if(0==set_value)
		   {
		     wdt_sys_val |= ~MTK_WDT_FAST_SWSYS_RST_MD_RST;
	  	...
	  	break;
	 case MTK_WDT_FAST_SWSYS_RST_MD_LITE_RST:
	  	   if(1==set_value)
	  	   {
	  	     wdt_sys_val |= MTK_WDT_FAST_SWSYS_RST_MD_LITE_RST;
	  	   }
		   if(0==set_value)
		   {
		     wdt_sys_val |= ~MTK_WDT_FAST_SWSYS_RST_MD_LITE_RST;
	...
	DRV_WriteReg32(MTK_WDT_FAST_SWSYSRST,wdt_sys_val);
	spin_unlock(&rgu_reg_operation_spinlock);

	mdelay(10);
	printk("after set wdt_sys_val =%x,wdt_sys_val=%x\n",DRV_Reg32(MTK_WDT_FAST_SWSYSRST),wdt_sys_val);
    return 0;
}
...
static int __init mtk_wdt_init(void)
{
	...
	printk(KERN_ALERT "mtk_wdt_init CONFIG_EMULATION_EARLY_PORTING entry!\n");
	unsigned int tmp;
...
MODULE_AUTHOR("MTK");
MODULE_DESCRIPTION("MT6571 Watchdog Device Driver");
MODULE_LICENSE("GPL");
