    ...
    if(cmd == SD_TOOL_ZONE) {
        ...
        if(id>=0 && id < HOST_MAX_NUM){
            if(p1 == 0){
                ...
                if(p3 >= 0 && p3< MSDC_CLKSRC_MAX_NUM){
...
static int msdc_debug_proc_read_FT(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    unsigned char  clk_drv,cmd_drv,dat_drv1,dat_drv2;
    u32 cur_rxdly0;
    ...
    if ( CONFIG_MTK_WCN_CMB_SDIO_SLOT == 0 ) {
        sdr_get_field(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, clk_drv);
        sdr_get_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DRVING, cmd_drv);
        sdr_get_field(MSDC0_DRVING_BASE, MSDC0_DAT0_3_DRVING, dat_drv1);
        sdr_get_field(MSDC0_DRVING_BASE, MSDC0_DAT4_7_DRVING, dat_drv2);
    } else if ( CONFIG_MTK_WCN_CMB_SDIO_SLOT == 1 ) {
        sdr_get_field(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, clk_drv);
        sdr_get_field(MSDC1_DRVING_BASE, MSDC1_CMD_DRVING, cmd_drv);
        sdr_get_field(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, dat_drv1);
    }
    ...
    p += sprintf(p, "mt65xx only using internal clock\n");

    p += sprintf(p, "-----------------------------------------\n");
    p += sprintf(p, "(3) settings of driving current \n");

    p += sprintf(p, "driving1(clk) is  <%d>\n", clk_drv);
    p += sprintf(p, "driving1(cmd) is  <%d>\n", cmd_drv);
    p += sprintf(p, "driving1(dat0~dat3) is  <%d>\n", dat_drv1);
    if ( msdc_id==0 ) {
...
static int msdc_debug_proc_write_FT(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    else if (i_case==2)//set driving current
    {
        ...
        if ( CONFIG_MTK_WCN_CMB_SDIO_SLOT == 0 ) {
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, i_driving);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DRVING, i_driving);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_DAT0_3_DRVING, i_driving);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_DAT4_7_DRVING, i_driving);
        } else if ( CONFIG_MTK_WCN_CMB_SDIO_SLOT == 1 ) {
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, i_driving);
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CMD_DRVING, i_driving);
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, i_driving);
        }
