...
#endif
char test_kdump[]={6,5,7,1,'k','d','u','m','p','t','e','s','t'};
//==============
#if MTK_MMC_DUMP_DBG
...
static struct simp_msdc_host g_msdc_host[HOST_MAX_NUM];
static struct simp_msdc_card g_msdc_card[HOST_MAX_NUM];
...
static struct simp_mmc_host g_mmc_host[HOST_MAX_NUM];
static struct simp_mmc_card g_mmc_card[HOST_MAX_NUM];
...
static unsigned int simp_mmc_power_up(struct simp_mmc_host *host,bool on)
{
    switch(host->mtk_host->id){
        case 0:
            ...
            break;
        case 1:
            /* for sd, makesure msdc host volt is the same as mt65xx IP internal LDO volt */
...
static unsigned int simp_mmc_enable_clk(struct simp_mmc_host *host)
{
    ...
    {
        sdr_write32(0xF0018124, 0x80F60000);
    }
    else // DDR3
    {
        sdr_write32(0xF0018124, 0x800CC000);
    }
#endif
...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
    ...
    switch (host->id) {
        case 0:
            ...
            sdr_set_field(MSDC0_SMT_CFG_BASE, MSDC0_SMT_ALL_MASK, 0xF);

            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, 3);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DRVING, 3);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_DAT0_3_DRVING, 3);
            sdr_set_field(MSDC0_DRVING_BASE, MSDC0_DAT4_7_DRVING, 3);
            break;
        case 1:
            ...
            sdr_set_field(MSDC1_SMT_CFG_BASE, MSDC1_SMT_ALL_MASK, 0xF);

            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, 3);
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_CMD_DRVING, 3);
            sdr_set_field(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, 3);
            break;
        default:
            break;
    }
...
static int simp_msdc_pio_read(struct simp_msdc_host *host, unsigned int *ptr, unsigned int size)
{
    ...
    unsigned int  left = size;
    unsigned int  status = 0;
    unsigned int  wints = MSDC_INTEN_DATTMO | MSDC_INTEN_DATCRCERR | MSDC_INTEN_XFER_COMPL;
    int err = 0;
    while(1){
        status = sdr_read32(MSDC_INT);
        sdr_write32(MSDC_INT,status);
        ...
        while (left) {
            ...
            if ( left!=0 ) {
                status = sdr_read32(MSDC_INT);
                status &= wints;
                if ( (status & MSDC_INT_DATTMO) || (status & MSDC_INT_DATCRCERR) ) {
                    sdr_write32(MSDC_INT,status);
                    if (status & MSDC_INT_DATTMO) err = -ETIMEDOUT;
                    if (status & MSDC_INT_DATCRCERR) err = -EIO;
