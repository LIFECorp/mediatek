...
void mt_set_pwm_clk_hal (U32 pwm_no, U32 clksrc, U32 div )
{
	...
	if (clksrc == CLK_BLOCK)
	{
		//CLRREG32 ( reg_con, 1 << 4 ); // If want to use 32K as clock source for old mode, remember to clear the bit here.
		CLRREG32 ( reg_con, 1 << PWM_CON_CLKSEL_OFFSET );
	}
	else if (clksrc == CLK_BLOCK_BY_1625_OR_32K)
	{
		//SETREG32 ( reg_con, 1 << 4 ); // If want to use 32K as clock source for old mode.
