...
void config_misc_info(int md_id, unsigned int base[], unsigned int size)
{
	...
	if(NULL != base) {
		...
		if(crystal_exist_status())
			...
			CCCI_MSG_INF(md_id,"ctl", "ENABLE_32K_CLK_LESS is defined. Crystal exist:%d, mask:%d.\n",
				crystal_exist_status(), misc_info.support_mask);
		#else
			misc_info.support_mask |= (FEATURE_NOT_SUPPORT<<(MISC_32K_LESS*2));
			CCCI_MSG_INF(md_id, "ctl", "ENABLE_32K_CLK_LESS is not defined. mask:%d.\n", misc_info.support_mask);
		#endif
...
static int send_tsx_temp(int md_id, char *buf, unsigned int len)
{
	int				ret = 0;
	unsigned int	msg;
	unsigned int	resv = 0;
	...
	msg = MD_SEND_TSX_TEMP;
	memcpy(&resv, buf, 4);
	ret = notify_md_by_sys_msg(md_id, msg, resv);
	CCCI_DBG_MSG(md_id, "ctl", "send tsx temp(%x) to md: %d\n", resv, ret);
	
	return ret;
}
EXPORT_SYMBOL(send_tsx_temp);
...
//	register_suspend_notify(md_id, SLP_ID_MD_FAST_DROMANT, md_fast_dormancy);
	...
	register_ccci_kern_func_by_md_id(md_id, ID_SEND_TSX_TEMP, send_tsx_temp);
