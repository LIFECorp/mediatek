...
void hal_rtc_bbpu_pwdn(void) 
{
	...
	DRV_WriteReg32((GPIO_BASE + 0x31c), ((0x09 << 16) | (0x09 << 20) | (0x09 << 24)));
	hal_rtc_xinfo("BBPU PWD 0x10005310=0x%X\n", DRV_Reg32(GPIO_BASE + 0x310));
	enable_clock(MT_CG_SPM_SW_CG, "BBPU_PWD");
	DRV_WriteReg32(SPM_POWERON_CONFIG_SET, 0x0B160001);
	...
	DRV_WriteReg32(SPM_POWER_ON_VAL1, 0x15030);
...
void hal_rtc_get_tick_time(struct rtc_time *tm) {
	u16 bbpu = rtc_read(RTC_BBPU) | RTC_BBPU_KEY | RTC_BBPU_RELOAD;
