...
#define SPI_CLOCK_PERIOD 66000  //kHz
...
static void enable_clk(void)
{
	...
	enable_clock(MT_CG_SPI_SW_CG, SPI_DRV_NAME);
#endif
	return;
}
...
{
#ifndef FPGA_EARLY_PORTING
	disable_clock(MT_CG_SPI_SW_CG, SPI_DRV_NAME);
#endif
	return;
}
...
#ifdef SPI_REC_DEBUG
		spi_speed = SPI_CLOCK_PERIOD/(chip_config->low_time + chip_config->high_time);
...
#ifdef SPI_VERBOSE
	u32 speed;
	speed= SPI_CLOCK_PERIOD/(chip_config->low_time + chip_config->high_time);
