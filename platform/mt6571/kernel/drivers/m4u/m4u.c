...
#define M4U_USE_ONE_PAGETABLE  //NO Support Secure Domain in MT6571
								...
								unsigned long a_Param)
{
    ...
    switch(a_Command)
    {
        ...
        case MTK_M4U_T_CACHE_SYNC :
            ...
            if (ret != 0)
                m4u_print_mva_list(a_pstFile, "in Cache_Sync");
        break;
            ...
            if(F_INT_PFH_DMA_FIFO_OVERFLOW != IntrSrc)        
            {
               if((IntrSrc&F_INT_TRANSLATION_FAULT) || IntrSrc&F_INT_INVALID_PHYSICAL_ADDRESS_FAULT)
               {
                   ...
                   M4UMSG("translation fault: port=%s, fault_mva=0x%x\n", m4u_get_port_name(larb_port_2_m4u_port(larbID, portID)), faultMva);
                    if (portID == M4U_PORT_CAM_IMGI)
                    {
                        M4UMSG("CAM_IMGI: 0x4230=0x%x 0x4234=0x%x", M4U_ReadReg32(LARB0_BASE, 0x4230), M4U_ReadReg32(LARB0_BASE, 0x4234));
                        M4UMSG("CAM_IMGI: 0x4238=0x%x 0x423C=0x%x", M4U_ReadReg32(LARB0_BASE, 0x4238), M4U_ReadReg32(LARB0_BASE, 0x423C));
                        M4UMSG("CAM_IMGI: 0x4240=0x%x 0x4248=0x%x", M4U_ReadReg32(LARB0_BASE, 0x4240), M4U_ReadReg32(LARB0_BASE, 0x4248));
                    }
               }
...
static char M4U_HELP[] =
    ...
    "             stop/start monitor\n"     
    "        cache:[0|1]\n"
    "             0: force enable all cache flush\n"         
    "             1: force enable range cache flush\n"             
    "        test_level:[0|1|2]\n"
    "             0: Set level to user\n"         
    "             1: Set level to eng\n"      
    ...
    "             read register\n"
    "        intr_hang:[0|1]\n"
    "             0: disable interrupt hang\n"
    "             1: enable interrupt hang\n"
    "\n";
...
static void process_dbg_opt(const char *opt)
{
    ...
    else if (0 == strncmp(opt, "intr_hang:", 10))
    {
        char *p = (char *)opt + 10;
        unsigned int enable = (unsigned int) simple_strtoul(p, &p, 10);
        if (1 == enable)
        {
            m4u_enable_error_hang(true);
            sprintf(buf, "Enable Intr Hang\n");
        }
        else
        {
            m4u_enable_error_hang(false);
            sprintf(buf, "Disable Intr Hang\n");
        }        
    }
...
static int m4u_proc_show(struct seq_file *seq, void *v)
{
    ...
    seq_printf(seq, "\nM4U PageTable base addr: VA=0x%x, PA=0x%x\n", pPT_nonsec, virt_to_phys(pPT_nonsec));
...
{  
    ...
    M4UMSG("module   mva_start  mva_end  block_num \n");
    for(index=1; index<MVA_MAX_BLOCK_NR+1; index += nr)
    {
        addr = index << MVA_BLOCK_SIZE_ORDER;
        nr = MVA_GET_NR(index);
...
#else
            M4UMSG("%s,   0x%-8x, 0x%-8x, %d\n", 
                m4u_get_module_name(moduleGraph[index]), addr, addr+mvasizeGraph[index], nr);
#endif			
        }
    }
...
{	//MT6571, M4U and SMI infra clock follow MM MTCMOS
    return 0;
}
								  ...
								  const unsigned int BufSize)
{
    ...
    for(s=1; (s<(MVA_MAX_BLOCK_NR+1))&&(mvaGraph[s]<nr); s+=(mvaGraph[s]&MVA_BLOCK_NR_MASK))
        ;
    if(unlikely(s > MVA_MAX_BLOCK_NR))
    {
        spin_unlock(&gMvaGraph_lock);
...
//	unsigned int align_page_num;
...
#ifdef M4U_4M_PAGETABLE
    {
        ...
        dma_handle = dma_map_single(NULL, pteStart, page_num<<2, DMA_TO_DEVICE);
                                  ...
                                  unsigned int entryCount)
{
    ...
    {
        M4UDBG("seq_range too small: %d, module:%s, MVAStart:0x%x, MVAEnd:0x%x, entryCount=%d", MVAEnd - MVAStart,
    ...
    if(!mva_owner_match(eModuleID, mva2module(MVAStart)))
    {
        ...
        M4UERR_WITH_OWNER("m4u_insert_seq_range module=%s, MVAStart=0x%x is %s, MVAEnd=0x%x is %s", m4u_get_module_name(eModuleID), m4u_get_module_name(eModuleID),
...
{ 
    M4ULOG("m4u_invalid_seq_range, module:%s, MVAStart:0x%x, MVAEnd:0x%x", m4u_get_module_name(eModuleID), MVAStart, MVAEnd);
	      
    if(!mva_owner_match(eModuleID, mva2module(MVAStart)))
    {
        M4UERR_WITH_OWNER("m4u_invalid_seq_range module=%s, MVAStart=0x%x is %s, MVAEnd=0x%x is %s", m4u_get_module_name(eModuleID),
    	    m4u_get_module_name(eModuleID), MVAStart, m4u_get_module_name(mva2module(MVAStart)),
    	    MVAEnd, m4u_get_module_name(mva2module(MVAEnd)));
        m4u_dump_mva_info();
    }
...
int m4u_config_port(M4U_PORT_STRUCT* pM4uPort) //native
{
    ...
    {
#ifdef M4U_USE_ONE_PAGETABLE	
        M4UERR("Error: MT6571 don't support secure domain!");
#else
    ...
    //switch on MM MTCMOS by switch on any MM clock
...
static int m4u_put_unlock_page(struct page* const page)
{
    ...
    pfn = page_to_pfn(page) - PHYS_PFN_OFFSET;
...
int m4u_dma_cache_maint(const M4U_MODULE_ID_ENUM eModuleID, const void *start, const size_t size, const int direction)
{
    ...
    {
        M4ULOG("Buffer align error: module=%s,addr=0x%x,size=%d,align=%d\n", 
            m4u_get_module_name(eModuleID), 
            (unsigned int)start, size, L1_CACHE_BYTES);
        M4ULOG("error: addr un-align, module=%s, addr=0x%x, size=0x%x, process=%s, align=0x%x\n", 
  	        m4u_get_module_name(eModuleID), (unsigned int)start, size, current->comm, L1_CACHE_BYTES);
    }
#endif
    ...
    if((unsigned int)start<PAGE_OFFSET)  // from user space
    {
        ret = __m4u_cache_sync_user(start, size, direction);
    }
    else
    {
...
#ifdef M4U_4M_PAGETABLE
static struct page* m4u_alloc_pagetable()
{
...
//=============================================
    ...
    // domain in MMsys always set 3, for debug purpose.
    COM_WriteReg32(REG_SMI_SECUR_CON(0), 0x66666666);
    COM_WriteReg32(REG_SMI_SECUR_CON(1), 0x66666666);
    COM_WriteReg32(REG_SMI_SECUR_CON(2), 0x6);
...
//===============================
        ...
        for(i=0; i<SMI_LARB_NR; i++)
        {
            ...
            //in 6571, SMI_SHARE_EN is not work.
...
static char* m4u_get_port_name(const M4U_PORT_ID_ENUM portID)
{
    switch(portID)
    {
        ...
        case M4U_PORT_CAM_IMGO              : return "CAM_IMGO";            
        case M4U_PORT_CAM_IMG2O             : return "CAM_IMG2O";
        case M4U_PORT_CAM_LSCI              : return "CAM_LSCI";
        ...
        case M4U_PORT_CAM_IMGI              : return "CAM_IMGI";            
        case M4U_PORT_CAM_ESFKO             : return "CAM_ESFKO";            
        case M4U_PORT_CAM_AAO               : return "M4U_PORT_CAM_AAO";            		
        case M4U_PORT_VENC_MVQP             : return "VENC_MVQP";            
        case M4U_PORT_VENCMC                : return "VENC_MC";            
