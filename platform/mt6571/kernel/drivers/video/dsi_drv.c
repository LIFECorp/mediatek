...
static DSI_PLL_CONFIG pll_config_debug = {0, 0x01, 0x00, 0x44000000, 1, 0x01B1, 0x066D, 0x066D};

static bool MIPIPLL_Debug = false;
static bool MIPIPLL_Debug_SSC_enable = true;
...
void DSI_SetMIPIPLL_Debug(bool enable, char *parameters)
{
    printk("mipipll_debug %s\n", enable ? "on" : "off");
    ...
    if (enable)
    {
        ...
        while ((para = strsep(&parameters, ",")) != NULL)
        {
            if (strncmp(para, "SSC=", 4) == 0)
            {
                char *p = (char *)para + 4;
                if (strncmp(p, "on", 2) == 0)
                {
                    MIPIPLL_Debug_SSC_enable = true;
                }
                else if (strncmp(p, "off", 3) == 0)
                {
                    MIPIPLL_Debug_SSC_enable = false;
                }
            }
            else if (strncmp(para, "TXDIV0=", 7) == 0)
            {
                char *p = (char *)para + 7;
                pll_config_debug.TXDIV0 = (unsigned int) simple_strtoul(p, &p, 16);
            }
            else if (strncmp(para, "TXDIV1=", 7) == 0)
            {
                char *p = (char *)para + 7;
                pll_config_debug.TXDIV1 = (unsigned int) simple_strtoul(p, &p, 16);
            }
            else if (strncmp(para, "SDM_PCW=", 8) == 0)
            {
                char *p = (char *)para + 8;
                pll_config_debug.SDM_PCW = (unsigned int) simple_strtoul(p, &p, 16);
            }
            else if (strncmp(para, "SSC_PH_INIT=", 12) == 0)
            {
                char *p = (char *)para + 12;
                pll_config_debug.SSC_PH_INIT = (unsigned int) simple_strtoul(p, &p, 16);
            }
            else if (strncmp(para, "SSC_PRD=", 8) == 0)
            {
                char *p = (char *)para + 8;
                pll_config_debug.SSC_PRD = (unsigned int) simple_strtoul(p, &p, 16);
            }
            else if (strncmp(para, "SSC_DELTA1=", 11) == 0)
            {
                char *p = (char *)para + 11;
                pll_config_debug.SSC_DELTA1 = (unsigned int) simple_strtoul(p, &p, 16);
            }
            else if (strncmp(para, "SSC_DELTA=", 10) == 0)
            {
                char *p = (char *)para + 10;
                pll_config_debug.SSC_DELTA = (unsigned int) simple_strtoul(p, &p, 16);
            }
        }

        printk("SSC         = %s\n", MIPIPLL_Debug_SSC_enable ? "on" : "off");
        printk("TXDIV0      = 0x%08X\n", pll_config_debug.TXDIV0);
        printk("TXDIV1      = 0x%08X\n", pll_config_debug.TXDIV1);
        printk("SDM_PCW     = 0x%08X\n", pll_config_debug.SDM_PCW);
        printk("SSC_PH_INIT = 0x%08X\n", pll_config_debug.SSC_PH_INIT);
        printk("SSC_PRD     = 0x%08X\n", pll_config_debug.SSC_PRD);
        printk("SSC_DELTA1  = 0x%08X\n", pll_config_debug.SSC_DELTA1);
        printk("SSC_DELTA   = 0x%08X\n", pll_config_debug.SSC_DELTA);
    }
    else
    {
        MIPIPLL_Debug = false;
...
void DSI_PHY_clk_setting(LCM_PARAMS *lcm_params)
{
    ...
    if (MIPIPLL_Debug)
    {
        memcpy(&pll_config_value, &pll_config_debug, sizeof(DSI_PLL_CONFIG));
        if (MIPIPLL_Debug_SSC_enable)
        {
            pll_sdm_ssc_en = 1;
        }
        else
...
void DSI_PHY_TIMCONFIG(LCM_PARAMS *lcm_params)
{
    ...
    if (MIPIPLL_Debug)
    {
        div1 = pll_config_debug.TXDIV0;
        div2 = pll_config_debug.TXDIV1;
        fbk_div = pll_config_debug.SDM_PCW >> 26;
    }
...
DSI_STATUS DSI_PS_Control(unsigned int ps_type, unsigned int vact_line, unsigned int ps_wc)
{
    ...
    OUTREGBIT(DSI_HSTX_CKLP_WC_REG, DSI_REG->DSI_HSTX_CKLP_WC, HSTX_CKLP_WC_AUTO, 1);
...
void DSI_BLS_Enable(bool enable)
{
	...
	if(enable){
		...
		OUTREG32(DISP_REG_BLS_EN, 0x00000001);
		OUTREG32(DISP_REG_BLS_DEBUG, 0x0);
		//disp_path_get_mutex_(gMutexID);
		DISP_REG_SET(DISP_REG_PQ_CTRL, 0x1);
		//disp_path_release_mutex_(gMutexID);
	}
	else{
		OUTREG32(DISP_REG_BLS_DEBUG, 0x3);
		OUTREG32(DISP_REG_BLS_EN, 0x00000000);
		OUTREG32(DISP_REG_BLS_DEBUG, 0x0);
		//disp_path_get_mutex_(gMutexID);
		DISP_REG_SET(DISP_REG_PQ_CTRL, 0x0);
		//disp_path_release_mutex_(gMutexID);
	}
}
	...
	switch(lane_num)
		{
			...
			case LCM_FOUR_LANE:ASSERT(0);break;
		}
	 OUTREG32(&DSI_REG->DSI_TXRX_CTRL, AS_UINT32(&tmp_reg));
