...
bool mt_usb_is_device(void)
{
	...
	if(!mtk_musb){
		...
		return false; // always return true for device mode for charging timing issue ALPS01262215 // usb need to return false for usb module not init done, otherwise charging will detect error type, ALPS01273682
	} else {
		DBG(4,"is_host=%d\n",mtk_musb->is_host);
	}
