...
static struct mt_gpufreq_power_info mt_gpufreqs_golden_power[] = {
    {.gpufreq_khz = GPU_DVFS_F0, .gpufreq_volt = GPU_VCORE_V0, .gpufreq_power = 377},
    {.gpufreq_khz = GPU_DVFS_F1, .gpufreq_volt = GPU_VCORE_V0, .gpufreq_power = 318},
    {.gpufreq_khz = GPU_DVFS_F0, .gpufreq_volt = GPU_VCORE_V1, .gpufreq_power = 317},
    {.gpufreq_khz = GPU_DVFS_F1, .gpufreq_volt = GPU_VCORE_V1, .gpufreq_power = 267},
    {.gpufreq_khz = GPU_DVFS_F2, .gpufreq_volt = GPU_VCORE_V0, .gpufreq_power = 246},
...
************************************************************/
static int mt_gpufreq_set(unsigned int old_freq, unsigned int target_freq, unsigned int old_volt, unsigned int target_volt)
{
...
*************************************************/
unsigned int mt_gpufreq_check_power(unsigned int gpu_pwr)
{
	unsigned int j,ret_gpupwr=0,gpufreq_power=0;
    dprintk("mt_gpufreq_check_power,gpu_pwr=%d\n",gpu_pwr);

    for(j = 0; j < mt_gpufreqs_power_num ; j++){
        if(mt_gpufreqs_power[j].gpufreq_volt == get_cur_vcore()){
            if(mt_gpufreqs_power[j].gpufreq_power <= gpu_pwr){
	            dprintk("mt_gpufreqs_power[%d].gpufreq_power: %d\n",j, mt_gpufreqs_power[j].gpufreq_power);
				gpufreq_power = mt_gpufreqs_power[j].gpufreq_power;
	        	dprintk("gpufreq_power: %d\n",gpufreq_power);
                break;
                //return mt_gpufreqs_power[j].gpufreq_power;
            }
        }
    }

	ret_gpupwr = gpu_pwr - gpufreq_power;
    dprintk("mt_gpufreq_check_power,%d - %d = %d\n",gpu_pwr,gpufreq_power, ret_gpupwr);
...
unsigned int mt_gpufreq_max_power(void)
{
    ...
    for(i = 0; i < mt_gpufreqs_power_num ; i++){
        if(mt_gpufreqs_power[i].gpufreq_volt == get_cur_vcore()){
            dprintk("mt_gpufreq_max_power: %d\n", mt_gpufreqs_power[i].gpufreq_power);
