...
const char FQMTR_source_name[NUM_OF_FQMTR_SOURCE][32] = {	"RESERVED",
															...
															"APMCU_CLOCK",
															"RTC_32KHZ_CLOCK",
															"FQMTR_SRC_RESERVED", /* reserved */
															"FQMTR_SRC_NFI_2X_CLOCK",
};
...
void gpiodbg_monitor(void)
{
	...
	reg = 0x0000FFFF & dcm_read_reg (GPIO_AUX_MODE(5));
	reg |= 0x66660000;
	dcm_write_reg(reg, GPIO_AUX_MODE(5));

	dcm_write_reg(0x66666666, GPIO_AUX_MODE(6));

	reg = 0x0ffff0000 & dcm_read_reg (GPIO_AUX_MODE(7));
	reg |= 0x00006666;
	dcm_write_reg(reg, GPIO_AUX_MODE(7));
}
