...
#define GPIOEXT_WR(addr, data)   pwrap_write((unsigned int)addr, data)
#define GPIOEXT_RD(addr)         ({ \
		unsigned int ext_data; \
		(pwrap_read((unsigned int)addr,&ext_data) != 0)?-1:ext_data;})
#define GPIOEXT_SET_BITS(BIT,REG)   (GPIOEXT_WR(REG, (unsigned int)(BIT)))
#define GPIOEXT_CLR_BITS(BIT,REG)    ({ \
		unsigned int ext_data; \
		int ret; \
		ret = GPIOEXT_RD(REG);\
		ext_data = ret;\
		(ret < 0)?-1:(GPIOEXT_WR(REG,ext_data & ~((unsigned int)(BIT))))})  
