...
int parse_img_header(unsigned int *start_addr, unsigned int img_num) //<<<------
{
    ...
    printf("parse_img_header img_num:%d\n", img_num);
	...
	for(i=0; i<img_num; i++) //<<<------
	{
		if(img_info_start[i].info.magic != PART_HEADER_MAGIC) {
            printf("parse_img_header error magic:0x%x\n", img_info_start[i].info.magic);
			continue;
        }
...
{
// modem debug register and bit: for mt6571 bringup
#define MD_DEBUG_MODE				(0x1000142C)
#define MD_DBG_JTAG_BIT				(1<<1)
	...
	switch(boot_md_id){
		case 0:// For MD1
			...
			// if debug register has been set debug mode, wait to boot modem manual.
			if ((*((volatile unsigned int*)(MD_DEBUG_MODE))) & MD_DBG_JTAG_BIT) {
				printf("md_boot_up: Do NOT to boot modem. read 0x1000142C:0x%x\n",
					(*((volatile unsigned int*)(MD_DEBUG_MODE))));
				*((volatile unsigned int*)(MD_DEBUG_MODE)) &= (~MD_DBG_JTAG_BIT);
				printf("md_boot_up: Do NOT to boot modem. cleared 0x1000142C:0x%x\n",
					(*((volatile unsigned int*)(MD_DEBUG_MODE))));
				break;
			}
