...
u32 mmc_init_device(void)
{
    ...
    if (!blkdev_get(BOOTDEV_SDMMC)) {
        ...
        //g_mmc_bdev.blkbuf  = (u8 *) storage_buffer;
        g_mmc_bdev.blkbuf  = NULL; //Use NULL and let blkdev_read()/blkdev_write() to assign buffer address 
                                   //  to prevent keeping a storage_buffer address before it is initialized and correctly set
...
int mmc_legacy_init(int verbose)
{
    ...
    //msdc_hard_reset(); //Comment out Temp
