...
//Since clock source index is not sorted according to clock frequency,
    ...
    switch (host->id) {
        case 0:
            if (set_smt)
                ...
                MSDC_WRITE32(reg_addr, MSDC0_SMT_ALL_MASK);
            #else
                MSDC_WRITE32(reg_addr, MSDC0_SMT_CLK|MSDC0_SMT_CMD_RST|MSDC0_SMT_DAT0_3);
            #endif
            break;
...
void msdc_set_rdtdsel(struct mmc_host *host,bool sd_18)
{
    switch (host->id) {
        case 0:
            if(sd_18){
                MSDC_SET_FIELD(MSDC0_TDSEL_BASE, MSDC0_TDSEL_ALL_MASK ,0xaaaa);
                MSDC_SET_FIELD(MSDC0_RDSEL_BASE, MSDC0_RDSEL_ALL_MASK ,0x0);
            } else{
                MSDC_SET_FIELD(MSDC0_TDSEL_BASE, MSDC0_TDSEL_ALL_MASK ,0xaaaa);
                MSDC_SET_FIELD(MSDC0_RDSEL_BASE, MSDC0_RDSEL_ALL_MASK ,0x30c30c);
            }
            break;
        case 1:
            if(sd_18){
                MSDC_SET_FIELD(MSDC1_TDSEL_BASE, MSDC1_TDSEL_ALL_MASK ,0xaaa);
                MSDC_SET_FIELD(MSDC1_RDSEL_BASE, MSDC1_RDSEL_ALL_MASK ,0x0);
            } else{
                MSDC_SET_FIELD(MSDC1_TDSEL_BASE, MSDC1_TDSEL_ALL_MASK ,0xaaa);
                MSDC_SET_FIELD(MSDC1_RDSEL_BASE, MSDC1_RDSEL_ALL_MASK ,0xc30c);
            }
            break;
        default:
            break;
...
void msdc_set_driving(struct mmc_host *host, u8 clkdrv, u8 cmddrv, u8 datdrv)
{
    ...
    switch (host->id) {
        //CLK, CMD, DAT3~0 share one driving setting
        case 0:
            MSDC_SET_FIELD(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, clkdrv);
            MSDC_SET_FIELD(MSDC0_DRVING_BASE, MSDC0_DAT0_3_DRVING, datdrv);
            #if defined(FEATURE_MMC_BOOT_MODE)
            MSDC_SET_FIELD(MSDC0_DRVING_BASE, MSDC0_CMD_RST_DRVING, cmddrv);
            MSDC_SET_FIELD(MSDC0_DRVING_BASE, MSDC0_DAT4_7_DRVING, datdrv);
            #endif
        ...
        #if defined(MMC_MSDC_DRV_CTP) || defined(MEM_PRESERVED_MODE_ENABLE)
        case 1:
            MSDC_SET_FIELD(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, clkdrv);
            MSDC_SET_FIELD(MSDC1_DRVING_BASE, MSDC1_CMD_DRVING, cmddrv);
            MSDC_SET_FIELD(MSDC1_DRVING_BASE, MSDC1_DAT0_3_DRVING, datdrv);
            break;
        #endif
...
void msdc_pad_ctrl_init(struct mmc_host *host)
{
    ...
    switch (host->id) {
        case 0:
            MSDC_SET_FIELD(MSDC0_PULL_EN_CFG_BASE, MSDC0_PULL_EN_ALL_MASK, 0x255556);
            ...
            //
            MSDC_SET_FIELD(MSDC0_GPIO_MODE9_ADDR,
            ...
            //
            MSDC_SET_FIELD(MSDC0_GPIO_MODE10_ADDR,
        ...
        #if defined(MMC_MSDC_DRV_CTP) || defined(MEM_PRESERVED_MODE_ENABLE)
        case 1:
            MSDC_WRITE32(MSDC1_IES_CFG_SET, MSDC1_IES_ALL_MASK); //Cool's solution for SD card: Enable IES bit
            //
            ...
            MSDC_WRITE32(MSDC1_PULL_EN_CFG_SET,
                     ...
                     MSDC1_DAT0_PULL50K_EN | MSDC1_DAT1_PULL50K_EN |
                     MSDC1_DAT2_PULL50K_EN | MSDC1_DAT3_PULL50K_EN);
            ...
            MSDC_SET_FIELD(MSDC1_CMD_PINMUX_ADDR,
            ...
            MSDC_SET_FIELD(MSDC1_DAT3_PINMUX_ADDR, MSDC1_DAT3_PINMUX_BITS, 0x1);

            // Set CLK pinmux after all other pinmux have been set
            // set gpio to msdc mode. (clk in GPIO_MODE10)
            MSDC_SET_FIELD(MSDC1_CLK_PINMUX_ADDR, MSDC1_CLK_PINMUX_BITS, 0x1);
            break;
        #endif
...
void msdc_save_pin_mux(struct mmc_host *host)
{
    switch (host->id) {
        case 0:
            ...
            host->default_reg[4] = MSDC_READ32(MSDC0_GPIO_MODE9_ADDR);
            host->default_reg[5] = MSDC_READ32(MSDC0_GPIO_MODE10_ADDR);
            #endif
            break;
...
void msdc_restore_pin_mux(struct mmc_host *host)
{
    ...
    switch (host->id) {
        case 0:
            #if defined(FEATURE_MMC_BOOT_MODE)
            tmpReg = MSDC_READ32(MSDC0_GPIO_MODE9_ADDR);
            tmpReg &= 0;
            tmpReg |= (host->default_reg[4] & 0xFFFFFFFF);
            MSDC_WRITE32(MSDC0_GPIO_MODE9_ADDR, tmpReg);

            tmpReg = MSDC_READ32(MSDC0_GPIO_MODE10_ADDR);
            tmpReg &= (~(0xFFF));
            tmpReg |= (host->default_reg[5] & (0xFFF));
            MSDC_WRITE32(MSDC0_GPIO_MODE10_ADDR, tmpReg);

            tmpReg = MSDC_READ32(MSDC0_SMT_CFG_BASE);
            tmpReg &= (0xFFFFFFF0);
            tmpReg |= (host->default_reg[2] & (0xF));
            MSDC_WRITE32(MSDC0_SMT_CFG_BASE, tmpReg);

            tmpReg = MSDC_READ32(MSDC0_DRVING_BASE);
            tmpReg &= (~MSDC0_DRVING_ALL_MASK);
            tmpReg |= (host->default_reg[3] & (MSDC0_DRVING_ALL_MASK));
            MSDC_WRITE32(MSDC0_DRVING_BASE, tmpReg);

            tmpReg = MSDC_READ32(MSDC0_PULL_SEL_CFG_BASE);
            tmpReg &= (~(MSDC0_PULL_SEL_ALL_MASK));
            tmpReg |= (host->default_reg[1] & (MSDC0_PULL_SEL_ALL_MASK));
    ...
    switch(host->id){
        case 0:
            if (mode) {
                ...
                MSDC_SET_FIELD(MSDC0_PULL_SEL_CFG_BASE, MSDC0_PULL_SEL_ALL_MASK, 0x3FE);
            } else {
            ...
            MSDC_WRITE32(MSDC0_PULL_EN_CFG_SET, 0x255556);
            break;
        #if defined(MMC_MSDC_DRV_CTP) || defined(MEM_PRESERVED_MODE_ENABLE)
        case 1:
            if (mode) {
            ...
            MSDC_WRITE32(MSDC1_PULL_EN_CFG_SET, MSDC1_PULL_EN_ALL_MASK);
            break;
        #endif
        default:
            break;
...
void msdc_pin_pnul(struct mmc_host *host, int mode)
{
    ...
    switch(host->id){
        ...
        case 1:
            if ( !mode )
                MSDC_WRITE32(MSDC1_PULL_EN_CFG_CLR, MSDC1_PULL_EN_ALL_MASK);
            break;
        default:
            break;
    }
...
void msdc_config_clksrc(struct mmc_host *host, clk_source_t clksrc)
{
    ...
    #if defined(MMC_MSDC_DRV_PRELOADER) || defined(MMC_MSDC_DRV_LK)
    if ( host->id==0 ) {
        MSDC_SET_FIELD(CLK_GATING_ADDR, MSDC0_CLK_GATING_EN, 1);
    } else if ( host->id==1 ) {
        MSDC_SET_FIELD(CLK_GATING_ADDR, MSDC1_CLK_GATING_EN, 1);
    }
...
void msdc_config_clock(struct mmc_host *host, int ddr, u32 hz)
{
    ...
    //msdc_config_clksrc(host, MSDC_CLKSRC_DEFAULT); //Comment out by Yuchi in JB
...
void msdc_set_host_level_pwr(struct mmc_host *host, u32 on, u32 level)
{
    switch (host->id) {
        ...
        case 1:
            ...
            msdc_set_smt(host,1);
            msdc_set_rdtdsel(host,1);
            msdc_set_driving(host, msdc_cap_removable.clk_18v_drv, msdc_cap_removable.cmd_18v_drv, msdc_cap_removable.dat_18v_drv);
            break;
    ...
    while (1) {
        ...
        //if (size >= size_threshold_large)
        {
            if (MSDC_TXFIFOCNT() == 0) {
                ...
                udelay1(60);//for DDR write timeout error verification test case
...
int msdc_pio_bwrite(struct mmc_host *host, ulong dst, uchar *src, ulong nblks)
{
    ...
    MSG(OPS, "[SD%d] Write PIO data %d bytes to 0x%x\n", host->id, nblks * blksz, (unsigned int)dst);
...
int msdc_switch_volt(struct mmc_host *host, int volt)
{
    ...
    if ((MSDC_READ32(MSDC_PS) & ((1 << 24) | (0xF << 16))) == 0) {
        ...
        //Originally set to MSDC_PIN_PULL_NONE. Since not support PULL_NONE, use PULL_DOWN instead
...
int msdc_tune_cmdrsp(struct mmc_host *host, struct mmc_command *cmd)
{
    ...
    MSDC_GET_FIELD(MSDC_IOCON, MSDC_IOCON_RSPL, orig_rsmpl);
#if defined(FEATURE_MMC_FINE_CM_TUNING)
    MSDC_GET_FIELD(MSDC_PAD_TUNE, MSDC_PAD_TUNE_CMDRRDLY, orig_rrdly);
    MSDC_GET_FIELD(MSDC_PATCH_BIT1, MSDC_CMD_RSP_TA_CNTR, orig_cmdrtc);
    MSDC_GET_FIELD(MSDC_PATCH_BIT0, MSDC_INT_DAT_LATCH_CK_SEL, orig_dl_cksel);
#endif
    if ( host->id==0 ) {
        driving_max_steps=MSDC0_ODC_NUM_MAX;
        MSDC_GET_FIELD(MSDC0_DRVING_BASE, MSDC0_CLK_DRVING, drv);
    } else { //if ( host->id==1 )
        driving_max_steps=MSDC1_ODC_NUM_MAX;
        MSDC_GET_FIELD(MSDC1_DRVING_BASE, MSDC1_CLK_DRVING, drv);
    }
            ...
            do {
                ...
                for (rsmpl = orig_rsmpl; rsmpl < 2; rsmpl++) {    //20130118 Light Add
                    ...
                    /* for debugging */
                    {
                        u32 t_rsmpl;
#if defined(FEATURE_MMC_FINE_CM_TUNING)
                        u32 t_rrdly, t_dl_cksel,t_cmdrtc;
                        //u32 t_cksel;
...
#if defined(FEATURE_MMC_FINE_RD_TUNING)
        ...
        do {
            ...
            do {
                ...
                for (dsmpl = orig_dsmpl; dsmpl < 2; dsmpl++) {  //20130118 Light Add
                    ...
                    /* for debugging */
                    {
                        u32 t_dspl, t_wrrdly, t_d_cntr;
                        //u32 t_ddrdly, t_cksel, t_dl_cksel;
                ...
                //Designer: tuning of DATWRDLY has little effect (JB2 linux driver does not use it)
...
//Note: Yuchi said that LK/Preloader do not use it. However, preloader on JB2 calls this function.
#if defined(MMC_MSDC_DRV_PRELOADER) || defined(MMC_MSDC_DRV_LK)
...
static reg_desc_t msdc_reg_desc[] = {
    ...
    {OFFSET_MSDC_DMA_LEN  , {"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}, {"00000000000000000000000000000000"}}, //new register
    ...
    /* PAD_CTL0/1/2 deleted in MSDC IP for cost reason */
