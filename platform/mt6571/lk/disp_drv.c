...
const LCM_DRIVER *disp_drv_get_lcm_driver(const char *lcm_name)
{
   ...
   printf("[LCM Auto Detect], try to find driver for [%s]\n", (lcm_name==NULL)?"unknown":lcm_name);
   ...
   else
   {
      ...
      for(i = 0;i < lcm_count;i++)
      {
         ...
         printf("[LCM Auto Detect] [%d] - [%s]\t", i, (lcm->name==NULL)?"unknown":lcm->name);
...
BOOL disp_get_lcm_name_boot(char *cmdline)
{
   ...
   printf("[mtkfb] LCM is %sconnected\n", ((isLCMFound)?"":"not "));
   p += 2;
   q = p;
   while(*q != ' ' && *q != '\0')
      q++;
...
DISP_STATUS DISP_Init(UINT32 fbVA, UINT32 fbPA, BOOL isLcmInited)
{
   ...
   OUTREG32(DISP_REG_CONFIG_DISP_OVL_MOUT_EN, 0x1);  // OVL output path, [0]: DISP_RDMA, [1]: DISP_WDMA, [2]: DISP_PQ 
...
DISP_STATUS DISP_PowerEnable(BOOL enable)
{
    ...
    {
        printf("ERROR: Can't get sem_update_screen in DISP_PowerEnable()\n");
...
DISP_STATUS DISP_PanelEnable(BOOL enable)
{
   ...
   if (down_interruptible(&sem_update_screen)) {
      printf("ERROR: Can't get sem_update_screen in DISP_PanelEnable()\n");
      return DISP_STATUS_ERROR;
   }
...
DISP_STATUS DISP_SetBacklight(UINT32 level)
{
	...
	if (down_interruptible(&sem_update_screen)) {
		printf("ERROR: Can't get sem_update_screen in DISP_SetBacklight()\n");
		return DISP_STATUS_ERROR;
	}
...
DISP_STATUS DISP_SetBacklight_mode(UINT32 mode)
{
   ...
   if (down_interruptible(&sem_update_screen)) {
      printf("ERROR: Can't get sem_update_screen in DISP_SetBacklight_mode()\n");
      return DISP_STATUS_ERROR;
   }
...
DISP_STATUS DISP_SetPWM(UINT32 divider)
{
   ...
   if (down_interruptible(&sem_update_screen)) {
      printf("ERROR: Can't get sem_update_screen in DISP_SetPWM()\n");
      return DISP_STATUS_ERROR;
   }
...
DISP_STATUS DISP_UpdateScreen(UINT32 x, UINT32 y, UINT32 width, UINT32 height)
{
   ...
   if (down_interruptible(&sem_update_screen)) {
      printf("ERROR: Can't get sem_update_screen in DISP_UpdateScreen()\n");
      return DISP_STATUS_ERROR;
   }
...
UINT32 DISP_GetScreenWidth(void)
{
   ...
   {
      printf("WARNING!! get screen width before display driver inited!\n");
      return 0;
   }
}
...
UINT32 DISP_GetScreenHeight(void)
{
   ...
   {
      printf("WARNING!! get screen height before display driver inited!\n");
      return 0;
   }
}
...
UINT32 DISP_GetVRamSizeBoot(char *cmdline)
{
   ...
   {
      printf("%s, can't get lcm type, reserved memory size will be set as 0x800000\n", __func__);
      return 0xC00000;
   }   
   // Align vramSize to 1MB
   //
   ...
   printf("DISP_GetVRamSizeBoot: %u bytes[%dMB]\n", vramSize, (vramSize>>20));
...
DISP_STATUS DISP_Capture_Videobuffer(unsigned int pvbuf, unsigned int bpp, unsigned int video_rotation)
{
   DISP_FUNC();
   if (down_interruptible(&sem_update_screen)) {
      printf("ERROR: Can't get sem_update_screen in DISP_Capture_Videobuffer()\n");
      return DISP_STATUS_ERROR;
   }
...
BOOL DISP_EsdCheck(void)
{
   ...
   if (down_interruptible(&sem_update_screen)) {
      printf("ERROR: Can't get sem_update_screen in DISP_EsdCheck()\n");
      return FALSE;
   }
...
BOOL DISP_EsdRecover(void)
{
   ...
   if (down_interruptible(&sem_update_screen)) {
      printf("ERROR: Can't get sem_update_screen in DISP_EsdRecover()\n");
      return FALSE;
   }
