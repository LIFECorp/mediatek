...
void FT_MODEM_INFO_OP(FT_MODEM_REQ *pLocalBuf, char *pft_PeerBuf, kal_int16 ft_peer_len)
{
	...
	int CCCI_IOC_MD_RESET = 0;
	int CCCI_IOC_RELOAD_MD_TYPE = 0;
	int CCCI_IOC_GET_MD_IMG_EXIST = 0;
	int CCCI_IOC_GET_MD_TYPE = 0;

	if(MTK_ENABLE_MD5)
	{			
		CCCI_IOC_MD_RESET = _IO(CCCI_IOC_MAGIC, 0);
		CCCI_IOC_RELOAD_MD_TYPE = _IO(CCCI_IOC_MAGIC, 43);
		CCCI_IOC_GET_MD_IMG_EXIST = _IOR(CCCI_IOC_MAGIC, 41, unsigned int[2]);
		CCCI_IOC_GET_MD_TYPE = _IOR(CCCI_IOC_MAGIC, 42, unsigned int);	
		fd = md5_device_note;
	}
	else
	{	
		CCCI_IOC_MD_RESET = _IO(CCCI_IOC_MAGIC, 0);
		CCCI_IOC_RELOAD_MD_TYPE	= _IO(CCCI_IOC_MAGIC, 25);
		CCCI_IOC_GET_MD_IMG_EXIST = _IOR(CCCI_IOC_MAGIC, 30, unsigned int[4]);
		CCCI_IOC_GET_MD_TYPE = _IOR(CCCI_IOC_MAGIC, 31, unsigned int);
		fd = open("/dev/ccci_ioctl4", O_RDWR|O_NOCTTY|O_NDELAY );		
	}
	...
	{
		ft_cnf.status = META_FAILED;
		FT_LOG("[FTT_Drv:]Open device note fail %d",fd );
