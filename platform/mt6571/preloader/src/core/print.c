...
#define C_LOG_SRAM_BUF_SIZE (2048)
...
static char *log_ptr = (char*)log_sram_buf;
static char *log_hdr = (char*)log_sram_buf;
static char *log_end = (char*)log_sram_buf + (C_LOG_SRAM_BUF_SIZE);
static int  g_log_drambuf = 0;
...
{
    if (drambuf) {      // dram
        if ((g_log_disable) && (!g_log_drambuf)) {  //log disable and recoding in sram before
            ...
            log_ptr = log_hdr;
            if (buf_len) {  // some string in sram buffer, copy from sram to dram
                memcpy(log_hdr, buf_ptr, buf_len);
                log_ptr = log_hdr + buf_len;
            }
            if (g_log_miss_chrs) {  //if any miss char, print out the missed char number
                outstr("\n{MISS: ");
                outdec(g_log_miss_chrs);
                outstr(" chars}\n");
                g_log_miss_chrs = 0;
            }
        } else if (!g_log_disable) {    // if log is enable and (recording in sram/dram before)
            log_hdr = (char*)log_dram_buf;
            log_end = log_hdr + LOG_BUFFER_MAX_SIZE;
            log_ptr = log_hdr;
        }
