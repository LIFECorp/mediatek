...
static u16 usbdl_verify_da(u8* da_addr, u32 da_len, u32 sig_len)
{
    ...
    if (FALSE == seclib_sec_usbdl_enabled(TRUE)) {
        print("%sDA verify disable\n", MOD);
    } else {
        /* init download agent authentication key */
        if (da_auth_init() != 0) {
            print("%sDA init key fail\n", MOD);
        ...
        if (da_len > DA_RAM_LENGTH) {
            print("%sDA(0x%x) relocate size(0x%x) fail\n", MOD, da_len, DA_RAM_LENGTH);
        ...
        if (sec_auth(da_addr, (da_len - sig_len), sig_addr, sig_len) != 0) {
            print("%sDA verify fail\n", MOD);
            status = ERR_DA_IMAGE_SIG_VERIFY_FAIL;
        }
    }
    #else
    /* validate fail */
    print("%sDA verify fail(mem)\n", MOD);
    ...
    if (FALSE == seclib_sec_usbdl_enabled(TRUE)) {
        ...
        print("%sDA dwld is forbidden\n", MOD);
        status = ERR_DA_IMAGE_SIG_VERIFY_FAIL;
    }
#endif
#endif  //#if SEC_ENV_ENABLE
...
#define LGE_FAC_SV_LEN      128
	...
	da_info.arg2 = (u32)sizeof(DownloadArg_v2);
...
int usbdl_handler(struct bldr_comport *comport, u32 hshk_tmo_ms)
{
    ...
    if (usbdl_check_start_command(comport, hshk_tmo_ms) == FALSE) {
        printf("%sstart cmd handshake TMO (%dms)\n", MOD, hshk_tmo_ms);
        return -1;
    }
    ...
    while (1) {
        ...
        case CMD_WRITE32:
            ...
            break;
        case CMD_PWR_INIT:
            DBGMSG("%sCMD_PWR_INIT\n", MOD);
            usbdl_pwr_init();
            break;
        case CMD_PWR_DEINIT:
            DBGMSG("%sCMD_PWR_DEINIT\n", MOD);
            usbdl_pwr_deinit();
            break;
        case CMD_PWR_READ16:
            DBGMSG("%sCMD_PWR_READ16\n", MOD);
            usbdl_pwr_read16();
            break;
        case CMD_PWR_WRITE16:
            DBGMSG("%sCMD_PWR_WRITE16\n", MOD);
            usbdl_pwr_write16();
            break;
        default:
            DBGMSG("%sUnhandled CMD 0x%x\n", MOD, cmd);
            break;
        }
        DBGMSG("\n");
    }
