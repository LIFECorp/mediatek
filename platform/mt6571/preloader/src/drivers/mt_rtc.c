...
static U16 get_frequency_meter(U16 val, U16 measureSrc, U16 window_size)
{
	...
	RTC_Write(TOP_CKPDN1, RTC_Read(TOP_CKPDN1) | 0x20); //TOP_CKPDN1[5]=1, gated fixed clock, FQMTR clock PDN
...
static U16 eosc_cali(void)
{
	...
	print("EOSC_Cali: TOP_CKCON1=0x%x\n", RTC_Read(TOP_CKCON1));
	while( left<=(right) )
	{
		middle = (right + left) / 2;
		if(middle == left)
...
static bool rtc_lpd_init(void)
{
	...
	if (!Write_trigger())
		return false;
			...
			Write_trigger();			
			count++;
	} while((count <= RTC_LPD_INIT_RETRY) && (RTC_Read(RTC_CON) & RTC_CON_LPSTA_RAW));

	//print("lpd_init RTC_CON=0x%x, count=0x%x\n", RTC_Read(RTC_CON), count);
	return true;
}
...
bool rtc_boot_check(void)
{
	...
	if ((rtc_con & (RTC_CON_LPSTA_RAW | RTC_CON_LPRST)) ||
				 !(rtc_con & RTC_CON_LPEN)	||
				 check_mode_flag)
	{
...
//	Write_trigger();
	...
	print("new_spare0=0x%x,new_spare1=0x%x,new_spare2=0x%x,new_spare3=0x%x\n",
...
void rtc_save_2sec_stat(void)
{
	...
	print("rtc_save_2sec_stat cali=%d!\n", cali);
