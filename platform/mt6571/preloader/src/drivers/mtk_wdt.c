...
static void mtk_wdt_check_trig_reboot_reason(void)
{
    ...
    reboot_type = mtk_wdt_boot_check();
    if((reboot_type != WDT_BY_PASS_PWK_REBOOT) && (reboot_type != WDT_NORMAL_REBOOT))
    	print("WDT not trig reboot\n");
					...
					BOOL wdt_en )
{
	...
	//mdelay(100);
	print(" mtk_wdt_mode_config  WDT_MODE=%x,tmp:%x\n",DRV_Reg32(MTK_WDT_MODE), tmp);
