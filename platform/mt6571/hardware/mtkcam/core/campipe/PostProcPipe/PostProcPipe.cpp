...
//kk tset
    ...
    for (MUINT32 i = 0; i < vOutPorts.size(); i++)
    {
        ...
        if (0 == memOutPort->index)
        {
            ...
            dispo.eImgRot = static_cast<NSImageio::NSIspio::EImageRotation>(memOutPort->u4Rotation/90);
            dispo.eImgFlip = static_cast<NSImageio::NSIspio::EImageFlip>(memOutPort->u4Flip);
#else
