...
start(SensorParam const & rSensorParam, MUINT32 u4ShotCount)
{
    ...
    if( mbAllocError )
    {
        //memory cannot be fully allocated. finish CShot directly.
        handleNotifyCallback(ECamShot_NOTIFY_MSG_CSHOT_END, 0, 0); 
        handleNotifyCallback(ECamShot_NOTIFY_MSG_ERROR, 0, 0); 
        handleDataCallback(ECamShot_DATA_MSG_JPEG, 0, 0, 0, 0); 
        return MTRUE;
    }
...
stop()
{
    ...
    if( mbAllocError )
    {
        ...
        mp3AObj->sendCommand(ECmd_CaptureStart, 0);  
...
convertImage(ImgBufInfo const & rSrcImgBufInfo, ImgBufInfo const & rDstImgBufInfo, int rot)
{
    ...
    int src_usize = rSrcImgBufInfo.u4Stride[1] * rSrcImgBufInfo.u4ImgHeight / 2; //
    int src_vsize = rSrcImgBufInfo.u4Stride[2] * rSrcImgBufInfo.u4ImgHeight / 2; //
    ...
    switch(rSrcImgBufInfo.eImgFmt) {
        case eImgFmt_YV12:  
            ...
            src_size_list[1] = src_usize;
            src_size_list[2] = src_vsize;
...
querySensorRawImgBufInfo()
{
    ...
    if( allocImgMem("SensorRaw", eImgFmt, u4SensorWidth, u4SensorHeight, rawMem) )
    {
...
queryYuvRawImgBufInfo()
{
    ...
    if( allocImgMem("Yuv", eImgFmt, u4Width, u4Height, yuvMem) )
    {
...
queryJpegImgBufInfo()
{
    ...
    else 
    {
        ...
        if( allocImgMem("Jpeg", eImgFmt_JPEG, u4Width, u4Height, jpegMem) )
        {
...
queryPostViewImgInfo()
{
    ...
    if( allocImgMem("PostView", mShotParam.ePostViewFmt, mShotParam.u4PostViewWidth, mShotParam.u4PostViewHeight, postviewMem) )
    {
...
queryThumbYuvImgBufInfo()
{
    ...
    //
    memset(&thumbnailMem, 0, sizeof(IMEM_BUF_INFO));
    if( allocImgMem("ThumbYuv", eImgFmt_YV12, mJpegParam.u4ThumbWidth, mJpegParam.u4ThumbHeight, thumbnailMem) )
    {
...
queryThumbTempImgBufInfo()
{
    ...
    //
    memset(&thumbnailMem, 0, sizeof(IMEM_BUF_INFO));
    if( allocImgMem("ThumbTempYuv", eImgFmt_YV12, mJpegParam.u4ThumbWidth*4, mJpegParam.u4ThumbHeight*4, thumbnailMem) )
    {
