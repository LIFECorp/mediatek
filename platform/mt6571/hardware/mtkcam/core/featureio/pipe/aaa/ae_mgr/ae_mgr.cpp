...
AE_EASYTUNING_PLINE_INFO_T rEasytuningTable;
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        memcpy(&(rEasytuningTable.AEPLineInfomation), &(g_rAEPlineTable->AEPlineInfo), u4AEPlineInfoSize);
        memcpy(&(rEasytuningTable.AEPLineMapping), (g_rAEPlineTable->pAEModePLineMapping), u4AEPlineMappingSize);
        memcpy(&(rEasytuningTable.normalAFPlineInfo), &(g_rAEInitInput.rAEPARAM.strAFPLine), u4AFTableSize);
        memcpy(&(rEasytuningTable.ZSDAFPlineInfo), &(g_rAEInitInput.rAEPARAM.strAFZSDPLine), u4AFTableSize);
        memcpy(&(rEasytuningTable.AEPLineGainList), &(g_rAEPlineTable->AEGainList), u4GainTableSize);
        memcpy(&rAEPlineDebugInfo, &rEasytuningTable, rAEPlineDebugInfo.u4Size);
    } else {
