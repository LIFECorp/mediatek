...
MINT32 SensorHalImp::getSensorInfo(ACDK_SCENARIO_ID_ENUM mode[2])
{
    ...
    #if 0 //debug only
    {
        ...
        LOG_MSG("SensorPreviewResolutionX = %d,\n"
            "SensorPreviewResolutionY = %d,\n"
            "SensorFullResolutionX = %d,\n"
            "SensorFullResolutionY = %d,\n"
            "SensorClockFreq = %d,\n"
            "SensorCameraPreviewFrameRate = %d,\n"
            "SensorVideoFrameRate = %d,\n"
            "SensorStillCaptureFrameRate = %d,\n"
            "SensorWebCamCaptureFrameRate = %d,\n"
            "SensorClockPolarity = %d,\n"
            "SensorClockFallingPolarity = %d,\n"
            "SensorClockRisingCount = %d,\n"
            "SensorClockFallingCount = %d,\n"
            "SensorClockDividCount = %d,\n"
            "SensorPixelClockCount = %d,\n"
            "SensorDataLatchCount = %d,\n"
            "SensorHsyncPolarity = %d,\n"
            "SensorVsyncPolarity = %d,\n"
            "SensorInterruptDelayLines = %d,\n"
            "SensorResetActiveHigh = %d,\n"
            "SensorResetDelayCount = %d,\n"
            "SensroInterfaceType = %d,\n"
            "SensorOutputDataFormat = %d,\n"
            "SensorMIPILaneNumber = %d,\n"
            "CaptureDelayFrame = %d,\n" 
            "PreviewDelayFrame = %d,\n"   
            "VideoDelayFrame = %d,\n" 
            "YUVAwbDelayFrame = %d,\n"
            "YUVEffectDelayFrame = %d,\n"
            "SensorGrabStartX = %d,\n"
            "SensorGrabStartY = %d,\n" 
            "SensorDrivingCurrent = %d,\n"   
            "SensorMasterClockSwitch = %d,\n"
            "AEShutDelayFrame = %d,\n"
            "AESensorGainDelayFrame = %d,\n"
            "AEISPGainDelayFrame = %d,\n"
            "MIPIDataLowPwr2HighSpeedTermDelayCount = %d,\n"
            "MIPIDataLowPwr2HighSpeedSettleDelayCount = %d,\n"
            "MIPICLKLowPwr2HighSpeedTermDelayCount = %d,\n"
            "SensorWidthSampling = %d,\n"
            "SensorHightSampling = %d,\n"
            "SensorPacketECCOrder = %d\n\n",
            pInfo[i]->SensorPreviewResolutionX,
            pInfo[i]->SensorPreviewResolutionY,
            pInfo[i]->SensorFullResolutionX,
            pInfo[i]->SensorFullResolutionY,
            pInfo[i]->SensorClockFreq,              /* MHz */
            pInfo[i]->SensorCameraPreviewFrameRate,
            pInfo[i]->SensorVideoFrameRate,
            pInfo[i]->SensorStillCaptureFrameRate,
            pInfo[i]->SensorWebCamCaptureFrameRate,
            pInfo[i]->SensorClockPolarity,          /* SENSOR_CLOCK_POLARITY_HIGH/SENSOR_CLOCK_POLARITY_Low */
            pInfo[i]->SensorClockFallingPolarity,
            pInfo[i]->SensorClockRisingCount,       /* 0..15 */
            pInfo[i]->SensorClockFallingCount,      /* 0..15 */
            pInfo[i]->SensorClockDividCount,        /* 0..15 */
            pInfo[i]->SensorPixelClockCount,        /* 0..15 */
            pInfo[i]->SensorDataLatchCount,         /* 0..15 */
            pInfo[i]->SensorHsyncPolarity,
            pInfo[i]->SensorVsyncPolarity,
            pInfo[i]->SensorInterruptDelayLines,
            pInfo[i]->SensorResetActiveHigh,
            pInfo[i]->SensorResetDelayCount,
            pInfo[i]->SensroInterfaceType,
            pInfo[i]->SensorOutputDataFormat,
            pInfo[i]->SensorMIPILaneNumber,
            pInfo[i]->CaptureDelayFrame, 
            pInfo[i]->PreviewDelayFrame,   
            pInfo[i]->VideoDelayFrame, 
            pInfo[i]->YUVAwbDelayFrame,
            pInfo[i]->YUVEffectDelayFrame,
            pInfo[i]->SensorGrabStartX,
            pInfo[i]->SensorGrabStartY, 
            pInfo[i]->SensorDrivingCurrent,   
            pInfo[i]->SensorMasterClockSwitch, 
            pInfo[i]->AEShutDelayFrame,    	     /* The frame of setting shutter default 0 for TG int */
            pInfo[i]->AESensorGainDelayFrame,   /* The frame of setting sensor gain */
            pInfo[i]->AEISPGainDelayFrame,
            pInfo[i]->MIPIDataLowPwr2HighSpeedTermDelayCount, 
            pInfo[i]->MIPIDataLowPwr2HighSpeedSettleDelayCount, 
            pInfo[i]->MIPICLKLowPwr2HighSpeedTermDelayCount, 
            pInfo[i]->SensorWidthSampling,
            pInfo[i]->SensorHightSampling,  
            pInfo[i]->SensorPacketECCOrder);
    }
    #endif
...
MINT32 SensorHalImp::setCSI2Config(MINT32 enableCSI2)
{
    ...
    if((mSensorDev & SENSOR_DEV_MAIN) || (mSensorDev & SENSOR_DEV_ATV)){
        if((enableCSI2 ==  1) && (sensorInfo[0].SensroInterfaceType == SENSOR_INTERFACE_TYPE_MIPI)) {   // enable csi2
            ...
            ret = pSeninfDrv->setTg1CSI2(sensorInfo[0].MIPIDataLowPwr2HighSpeedTermDelayCount, 0x12/*sensorInfo[0].MIPIDataLowPwr2HighSpeedSettleDelayCount*/, 
    ...
    if(mSensorDev & SENSOR_DEV_SUB) {
        if((enableCSI2 ==  1) && (sensorInfo[1].SensroInterfaceType == SENSOR_INTERFACE_TYPE_MIPI)) {   // enable csi2
            // Set mipi csi2         
            LOG_MSG("enable Tg1 CSI2Config\n");
            ret = pSeninfDrv->setTg1CSI2(sensorInfo[1].MIPIDataLowPwr2HighSpeedTermDelayCount, 0x12/*sensorInfo[1].MIPIDataLowPwr2HighSpeedSettleDelayCount*/, 
...
MINT32 SensorHalImp::setSensorIODrivingCurrent()
{
    ...
    if(mSensorDev & SENSOR_DEV_MAIN) {   
        ret = pSeninfDrv->setTg1IODrivingCurrent(sensorInfo[0].SensorDrivingCurrent);
        if (ret < 0) {
    ...
    if(mSensorDev & SENSOR_DEV_SUB) {
        ret = pSeninfDrv->setTg1IODrivingCurrent(sensorInfo[1].SensorDrivingCurrent);
        if (ret < 0) {
