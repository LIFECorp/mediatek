...
ImgSensorDrv::impSearchSensor(pfExIdChk pExIdChkCbf)
{
    ...
    for (SensorEnum = DUAL_CAMERA_MAIN_SENSOR; SensorEnum <= DUAL_CAMERA_SUB_SENSOR; SensorEnum <<= 1)  {
        LOG_MSG("impSearchSensor search to main and sub\n");
   #else
    for (SensorEnum = DUAL_CAMERA_MAIN_SENSOR; SensorEnum < DUAL_CAMERA_SUB_SENSOR; SensorEnum <<= 1)  {
        LOG_MSG("impSearchSensor search to main only\n");   
   #endif     
        //skip atv case
            ...
            if (err < 0 || err2 < 0) {
                ...
                if (SensorEnum == DUAL_CAMERA_MAIN_SENSOR) {
                    ...
                    if  ( m_pMainSensorInfo )
                    {
                        ...
                        LOG_MSG("found <%#x/%s/%s>\n", pSensorInfo->GetID(), pSensorInfo->getDrvName(), pSensorInfo->getDrvMacroName());
                    }
                    else
                    {
