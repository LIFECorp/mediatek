...
static MBOOL gDumpIMemcLog = MFALSE;
...
#define LOG_INF(fmt, arg...)        do { if (gDumpIMemcLog) { BASE_LOG_INF(fmt, ##arg); } } while(0)
...
#if defined (__ISP_USE_LUT_MAP_PHYMEM__)
    //virtual MINT32  lookUpPhyAddr(MUINT32 u4VA);
	virtual MBOOL lookUpPhyAddr(MUINT32 u4VA, MUINT32* u4PA, int* index);		
    virtual MBOOL unmapAllPhyAddr(void);
#endif
...
#if defined (__ISP_USE_LUT_MAP_PHYMEM__)
        vector<IMEM_BUF_INFO> m_BufMapPA;
#endif
        MINT32 mIspFd;
