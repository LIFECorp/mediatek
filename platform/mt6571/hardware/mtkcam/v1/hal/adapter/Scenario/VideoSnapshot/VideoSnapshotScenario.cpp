...
//-----------------------------------------------------------------------------
static MBOOL isSameAspect(MUINT32 w1, MUINT32 h1, MUINT32 w2, MUINT32 h2, MUINT32 tolerance)
{
    //tolerance: in percent
    MUINT32 diff_th = h1 * h2 * tolerance / 100;
    MUINT32 val_0 = w1 * h2;
    MUINT32 val_1 = w2 * h1;
    MUINT32 diff = (val_0 > val_1) ? (val_0 - val_1) : (val_1 - val_0);

    return (diff < diff_th) ? MTRUE : MFALSE;
}
//-----------------------------------------------------------------------------
...
setImage(ImageInfo &img)
{
    ...
    //
    rHWCrop.x = img.crop.x;
    rHWCrop.y = img.crop.y;
    rHWCrop.w = img.crop.w;
    rHWCrop.h = img.crop.h;
    //
    mpParamsMgr->getVideoSize(&recWidth, &recHeight);
    //
    if( isSameAspect(rHWCrop.w, rHWCrop.h, recWidth, recHeight, 5) )
    {
    ...
    {
        //consider hrz is enabled
        if( rHWCrop.w < recWidth )
        {
            MY_LOGW("hrz enabled: Crop:W(%d)xH(%d) < Rec:W(%d)xH(%d)",
                    rHWCrop.w,
                    rHWCrop.h,
                    recWidth,
                    recHeight);
        ...
        {
            picWidth = rHWCrop.w;
            picHeight = rHWCrop.w * recHeight / recWidth;
            MY_LOGD("hrz enabled: pic W(%d)xH(%d)", picWidth, picHeight);
        }
    }
    //
...
transMainThumb(MBOOL en)
{
    ...
    if(en)
    {
        ...
        if(mIsThumb)
        {
            ...
            Config.DispoOut.Size.Stride[0] = FmtUtils::queryImgWidthStride(
                                                        CameraParameters::PIXEL_FORMAT_YUV422I,
                                                        Config.DispoOut.Size.Width,
                                                        0);
            Config.DispoOut.Size.Stride[1] = 0;
            Config.DispoOut.Size.Stride[2] = 0;
            ...
            Config.DispoOut.Mem.Size = FmtUtils::queryImgBufferSize(
                                                        CameraParameters::PIXEL_FORMAT_YUV422I,
                                                        Config.DispoOut.Size.Width,
                                                        Config.DispoOut.Size.Height);
#endif
        }
        else
        {
        ...
        Config.VidoOut.Size.Stride[0] = FmtUtils::queryImgWidthStride(
                                                    CameraParameters::PIXEL_FORMAT_YUV422I,
                                                    Config.VidoOut.Size.Width,
                                                    0);
        Config.VidoOut.Size.Stride[1] = 0;
        Config.VidoOut.Size.Stride[2] = 0;
...
#if (PLATFORM_VERSION_MAJOR == 2)
        Config.VidoOut.Mem.Size = FmtUtils::queryImgBufferSize(
                                                    CameraParameters::PIXEL_FORMAT_YUV422I,
                                                    Config.VidoOut.Size.Width,
                                                    Config.VidoOut.Size.Height);
#endif
...
rotateThumb(MBOOL en)
{
    ...
    if(en)
    {
        ...
        //
        Config.ImageIn.Size.Stride[0] = FmtUtils::queryImgWidthStride(
                                                    CameraParameters::PIXEL_FORMAT_YUV422I,
