...
#define THREAD_ATTR_NORMAL			{0, NULL, 1024 * 1024, 4096, SCHED_OTHER, -20}
...
EVBracketCapture(void)
{
	...
	if(ret) {
		...
		pthread_attr_t attr = THREAD_ATTR_NORMAL;
		pthread_create(&mCaptureIMemThread, &attr, HdrShot::allocateCaptureMemoryTask_All, this);
        #else
		...
		pthread_create(&mCaptureIMemThread, &attr, HdrShot::allocateCaptureMemoryTask_Others, this);
		#endif
	}
#endif
...
fgCamShotNotifyCb(MVOID* user, NSCamShot::CamShotNotifyInfo const msg)
{
    ...
    if (NULL != pHdrShot)
    {
        if (NSCamShot::ECamShot_NOTIFY_MSG_EOF == msg.msgType)
        {
			...
			pthread_attr_t attr = THREAD_ATTR_NORMAL;
        	pthread_create(&pHdrShot->mProcessIMemThread, &attr, HdrShot::allocateProcessMemoryTask, pHdrShot);
...
doHDRAlgoTask(MVOID* arg)
{
	FUNCTION_LOG_START;
	MBOOL 	ret = MTRUE;
	prctl(PR_SET_NAME,(long unsigned int)"Hdr doHDRAlgoTask", 0, 0, 0);
...
createNormalJpegImgTask(MVOID* arg)
{
	FUNCTION_LOG_START;
	MBOOL 	ret = MTRUE;
	prctl(PR_SET_NAME,(long unsigned int)"Hdr jpegImgTask", 0, 0, 0);
...
allocateCaptureMemoryTask_Others(MVOID* arg)
{
	...
	HdrShot *self = (HdrShot*)arg;
	prctl(PR_SET_NAME,(long unsigned int)"Hdr allocateCaptureMemoryTask_Others", 0, 0, 0);
	//non-real-time policy
	SetThreadProp(SCHED_OTHER, -20);
...
allocateProcessMemoryTask(MVOID* arg)
{
	...
	prctl(PR_SET_NAME,(long unsigned int)"Hdr allocateProcessMemoryTask", 0, 0, 0);
	//non-real-time policy
	SetThreadProp(SCHED_OTHER, -20);
...
saveFileTask(MVOID* arg)
{
	...
	prctl(PR_SET_NAME,(long unsigned int)"Hdr saveFileTask", 0, 0, 0);
	//non-real-time policy
	SetThreadProp(SCHED_OTHER, -20);
