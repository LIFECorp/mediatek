...
#define PQ_PURPTONE_H_NUM   (0)
#define PQ_SKINTONE_H_NUM   (0)
#define PQ_GRASSTONE_H_NUM  (0)
#define PQ_SKYTONE_H_NUM    (0)
#define PQ_PURPTONE_S_NUM   (0)
#define PQ_SKINTONE_S_NUM   (0)
#define PQ_GRASSTONE_S_NUM  (0)
#define PQ_SKYTONE_S_NUM    (0)
...
{  
    ...
    u4Indics[PQ_Sharp_Adj] = pqparam.u4SharpnessAdj;
    u4Indics[PQ_Contrast_Adj] = pqparam.u4ContrastAdj;
    u4Indics[PQ_Sat_Adj] = pqparam.u4SaturationAdj;
...
static jboolean setIndex(PQ_TuningIndex_t a_eIndex , unsigned int a_u4Index)
{
    ...
    pqparam.u4SharpnessAdj = u4Indics[PQ_Sharp_Adj];
    pqparam.u4ContrastAdj = u4Indics[PQ_Contrast_Adj];
    pqparam.u4SaturationAdj = u4Indics[PQ_Sat_Adj];

    if(drvID == -1) //initial
        drvID = open("/dev/mtk_disp", O_RDONLY, 0);
        
    XLOGE("Setting PQ param : [CDDispSharp %d CDContrast %d CDSat %d] \n",
        pqparam.u4SharpnessAdj, pqparam.u4ContrastAdj, pqparam.u4SaturationAdj);    
