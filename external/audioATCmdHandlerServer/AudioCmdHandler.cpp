...
ACHStatus AudioCmdHandler::DLCustAudioFLTParamToNV(void *pParam,AudioFltTunningType eFLTID)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        SXLOGV("Received  - loudness hsf coeffs bes_loudness_hsf_coeff_L[0][0][%d] = %d", i, pCustomPara->bes_loudness_hsf_coeff_L[0][0][i]);
    }

    for (int i = 0; i < 3; i++)
    {
        SXLOGV("Received - loudness bpf coeffs bes_loudness_bpf_coeff_L[0][0][%d]=%d", i, pCustomPara->bes_loudness_bpf_coeff_L[0][0][i]);
    }

    for (int i = 0; i < 3; i++)
    {
        SXLOGV("Received - loudness lpf coeffs bes_loudness_lpf_coeff_L[0][%d]=%d", i, pCustomPara->bes_loudness_lpf_coeff_L[0][i]);
    }

    for (int i = 0; i < 5; i++)
    {
        SXLOGV("Received  - loudness hsf coeffs bes_loudness_hsf_coeff_R[0][0][%d] = %d", i, pCustomPara->bes_loudness_hsf_coeff_R[0][0][i]);
    }

    for (int i = 0; i < 3; i++)
    {
        SXLOGV("Received - loudness bpf coeffs bes_loudness_bpf_coeff_R[0][0][%d]=%d", i, pCustomPara->bes_loudness_bpf_coeff_R[0][0][i]);
    }

    for (int i = 0; i < 3; i++)
    {
        SXLOGV("Received - loudness lpf coeffs bes_loudness_lpf_coeff_R[0][%d]=%d", i, pCustomPara->bes_loudness_lpf_coeff_R[0][i]);
    }
    ...
    SXLOGD("Received - AudioFLT Param bes_loudness_DRC_Delay = %d", pCustomPara->bes_loudness_DRC_Delay);
    SXLOGD("Received - AudioFLT Param bes_loudness_Lim_Th = %d", pCustomPara->bes_loudness_Lim_Th);
    SXLOGD("Received - AudioFLT Param bes_loudness_Lim_Gn = %d", pCustomPara->bes_loudness_Lim_Gn);
    SXLOGD("Received - AudioFLT Param bes_loudness_Lim_Const = %d", pCustomPara->bes_loudness_Lim_Const);
    SXLOGD("Received - AudioFLT Param bes_loudness_Lim_Delay = %d", pCustomPara->bes_loudness_Lim_Delay);
...
ACHStatus AudioCmdHandler::ULCustAudioFLTParamFromNV(void *pParam, int *len,AudioFltTunningType eFLTID)
{
    ...
    if (size != sizeof(AUDIO_ACF_CUSTOM_PARAM_STRUCT))
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            SXLOGV("Ori  - loudness hsf coeffs bes_loudness_hsf_coeff_L[0][0][%d] = %d", i, pACFPara->bes_loudness_hsf_coeff_L[0][0][i]);
        }
    
        for (int i = 0; i < 3; i++)
        {
            SXLOGV("Ori - loudness bpf coeffs bes_loudness_bpf_coeff_L[0][0][%d]=%d", i, pACFPara->bes_loudness_bpf_coeff_L[0][0][i]);
        }
    
        for (int i = 0; i < 3; i++)
        {
            SXLOGV("Ori - loudness lpf coeffs bes_loudness_lpf_coeff_L[0][%d]=%d", i, pACFPara->bes_loudness_lpf_coeff_L[0][i]);
        }

        for (int i = 0; i < 5; i++)
        {
            SXLOGV("Ori  - loudness hsf coeffs bes_loudness_hsf_coeff_R[0][0][%d] = %d", i, pACFPara->bes_loudness_hsf_coeff_R[0][0][i]);
        }
    
        for (int i = 0; i < 3; i++)
        {
            SXLOGV("Ori - loudness bpf coeffs bes_loudness_bpf_coeff_R[0][0][%d]=%d", i, pACFPara->bes_loudness_bpf_coeff_R[0][0][i]);
        }
    
        for (int i = 0; i < 3; i++)
        {
            SXLOGV("Ori - loudness lpf coeffs bes_loudness_lpf_coeff_R[0][%d]=%d", i, pACFPara->bes_loudness_lpf_coeff_R[0][i]);
        }
        ...
        SXLOGD("Ori - AudioFLT Param bes_loudness_DRC_Delay = %d", pACFPara->bes_loudness_DRC_Delay);
        SXLOGD("Ori - AudioFLT Param bes_loudness_Lim_Th = %d", pACFPara->bes_loudness_Lim_Th);
        SXLOGD("Ori - AudioFLT Param bes_loudness_Lim_Gn = %d", pACFPara->bes_loudness_Lim_Gn);
        SXLOGD("Ori - AudioFLT Param bes_loudness_Lim_Const = %d", pACFPara->bes_loudness_Lim_Const);
        SXLOGD("Ori - AudioFLT Param bes_loudness_Lim_Delay = %d", pACFPara->bes_loudness_Lim_Delay);
