...
int main (int argc, char **argv)
{
	...
	printf("%s",_help);
		...
		if(!strcmp(argv[1],"lcm_get_id"))
		   ...
		   if(id_num == 0)
			...
			sscanf(argv[3],"0X%x",&(timing.value));
			#endif
			timing.value[0] = atoi(argv[3]);
			timing.dsi_index= (DSI_INDEX)atoi(argv[4]);
			...
			{
			DSI_RET dsi_contin ={0} ;
			ioctl(fd, LCM_GET_DSI_CONTINU, &dsi_contin);
			printf("get_dsi_continuous=>dsi0:%d dsi1:%d\n",dsi_contin.dsi[0],dsi_contin.dsi[1]);
			}
		...
		BURST_VDO_MODE = 3
			...
			*/
			...
			printf("\nUsage: <./fbconfig get_dsi_timing VBP>\n");
				...
				printf("get_dsi_timing==>%s: dsi0=>%d dsi1=>%d\n",argv[2],timing.value[0],timing.value[1]);
				}
			}
	...
	{
	printf(" \n***open /sys/kernel/debug/fbconfig Fail!!*** \n");
	}
