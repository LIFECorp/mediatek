...
void (*backlight_control_setup)(void (*)(void), void (*)(void)) = NULL;
...
void loadlib()
{
        ...
        backlight_control_setup = (void (*)(void(*)(), void(*)()))
            dlsym(handle, "backlight_control_setup");
...
close_handle:
        dlclose(handle);
fail_out:
        sleep(3); // delay 3s for mobilelog to collect error logs
        property_set(ANDROID_RB_PROPERTY, "shutdown,fail to load libipod");
}
...
static void _start_backlight(void){
    ...
    if(binder == NULL){
        SXLOGE("failed to get power service");
        return;
    }
    sp<IPowerManager> pms = IPowerManager::asInterface(binder);
    if(pms == NULL){
        SXLOGE("fail to get power service");
        return;
    }
...
{    
	...
	if(params[PARAM_CHARGING_MODE] != 1){
        ...
        if(backlight_control_setup)
            backlight_control_setup(_start_backlight, _stop_backlight);
