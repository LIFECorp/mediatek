...
int ril_mmi_command_hdlr(char* line){
    int i = 0, sendLen = 0, dataLen = 0;;
    int table_size = 0;
    char response[MAX_AT_RESPONSE];	
    char mmiCmd[MAX_DATA_SIZE];	
    char* line_cut = cut_cmd_line(line);
    
    table_size = (sizeof(ril_mmi_cmd_table)/sizeof(ril_mmi_cmd_type));
    
    ALOGD("ril_mmi_command_hdlr :%s", line);
