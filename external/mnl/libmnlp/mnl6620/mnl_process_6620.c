...
static int mnlcmd_handler(int fd) /*sent from mnld*/
{
    ...
    else if ((cmd == MNL_CMD_ENABLE_AGPS_DEBUG) || (cmd == MNL_CMD_DISABLE_AGPS_DEBUG))
    {
	    MNL_MSG("Send the message to AGPSD here");
        if (cmd == MNL_CMD_ENABLE_AGPS_DEBUG)
            ret = mtk_gps_sys_agps_disaptcher_callback(MTK_AGPS_CB_AGPS_RAWDATA_CONT, 1, "1");
        if (cmd == MNL_CMD_DISABLE_AGPS_DEBUG)
            ret = mtk_gps_sys_agps_disaptcher_callback(MTK_AGPS_CB_AGPS_RAWDATA_CONT, 1, "0");
        if (ret == -1){
            MNL_MSG("Send command to AGPSD fail");
            char buff = {MNL_CMD_SWITCH_AGPS_DEBUG_FAIL};
            ret = sig_send_cmd(mnl_sinproc.dae_snd_fd, buff, sizeof(buff));
	    }else if (ret == 0){
            MNL_MSG("Send command to AGPSD done");
            char buff = {MNL_CMD_SWITCH_AGPS_DEBUG_DONE};			
            ret = sig_send_cmd(mnl_sinproc.dae_snd_fd, buff, sizeof(buff));
        }
        return ret;
    }
