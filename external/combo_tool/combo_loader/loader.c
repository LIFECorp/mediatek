...
static int insert_wmt_module_for_soc(int chipid, char *modulePath, char *nameBuf, int nameBufLen)
{
	...
	{
		printf("invalid parameter:modulePath(%p), nameBuf(%p), nameBufLen(%d)\n", modulePath, nameBuf, nameBufLen);
		return iRet;
	}
	...
	if ((0 != iRet) && (-1 != chipId))
	{
		...
		if (0x6630 == chipId)
		{
			...
			do {
				...
				{
					 // do nothing
					 printf("no external combo chip detected\n");
				}
				else
				{
				...
				if(0 == noextChip)
				{
					iRet = ioctl(gLoaderFd,COMBO_IOCTL_DO_SDIO_AUDOK,chipId);
					if (0 != iRet)
					{
