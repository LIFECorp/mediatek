...
static int read_section(struct ubigen_info *ui, const char *sname,
			...
			struct stat *st)
{
	...
	*img = NULL;
	...
	if(ui->device_size == 0) {
		sprintf(buf, "%s:ubi_device_size", sname);
		p = iniparser_getstring(args.dict, buf, NULL);
		if (p) {
			ui->device_size = atoi(p);
			if (ui->device_size <= 0)
				return errmsg("bad \"ubi_device_size\" key %s in section \"%s\"", p, sname);

			ui->peb_count = ui->device_size / ui->peb_size;
			verbose(args.verbose, "ubi_device_size: %d", ui->device_size);
			verbose(args.verbose, "peb_count: %d", ui->peb_count);
		} else {
			return errmsg("\"ubi_device_size\" key not found in section \"%s\"", sname);
		}
	}
	return 0;
}
...
int main(int argc, char * const argv[])
{
	 ...
	 */
	seek = ui.peb_size * 2;
#if 0 //#ifdef MTK_NAND_UBIFS_FASTMAP_SUPPORT
	//MTK: skip 1 PEB for save fastmap
	seek += ui.peb_size * 1;
	ui.used_peb+=3; //2 for vol tbl , 1 for skip space for fastmap
#endif
	...
	for (i = 0; i < sects; i++) {
		...
		if (img) {
			...
			vi[i].start = ui.used_peb; //MTK: save start PEB
			err = ubigen_write_volume(&ui, &vi[i], args.ec, st.st_size, fd, args.out_fd, &lnum);
			close(fd);
			if (err) {
			...
			ui.used_peb+=lnum; //MTK: count used PEB
			vi[i].stop = ui.used_peb; //MTK: save stop PEB
		} else {
			vi[i].start = -1;
			vi[i].stop = -1;
		}
...
#if 0 //#ifdef MTK_NAND_UBIFS_FASTMAP_SUPPORT
	ui.ec = args.ec;
	ui.fm_size = ubi_calc_fm_size(&ui);
	
        /*
         * fm_pool.max_size is 5% of the total number of PEBs but it's also
         * between UBI_FM_MAX_POOL_SIZE and UBI_FM_MIN_POOL_SIZE.
         */
        ui.fm_pool.max_size = min( (ui.peb_count / 100) * 5, UBI_FM_MAX_POOL_SIZE);
        if (ui.fm_pool.max_size < UBI_FM_MIN_POOL_SIZE)
                ui.fm_pool.max_size = UBI_FM_MIN_POOL_SIZE;

        ui.fm_wl_pool.max_size = UBI_FM_WL_POOL_SIZE;

        if (ui.peb_count <= UBI_FM_MAX_START) {
                verbose(args.verbose, "More than %i PEBs are needed for fastmap, sorry.",
                        UBI_FM_MAX_START);
                fm_disabled = 1;
        }

        verbose(args.verbose, "default fastmap pool size: %d", ui.fm_pool.max_size);
        verbose(args.verbose, "default fastmap WL pool size: %d", ui.fm_wl_pool.max_size);
	
	if(fm_disabled == 0) {
		ubi_update_fastmap(&ui, vi, i, args.out_fd);
	}
#endif
