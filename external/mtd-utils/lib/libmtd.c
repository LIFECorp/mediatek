	     ...
	     void *buf, int len)
{
	...
	seek = (off64_t)eb * mtd->eb_size + offs;
	if (lseek64(fd, seek, SEEK_SET) != seek)
		return sys_errmsg("cannot seek mtd%d to offset %llu, eb %d offset %d",
				  mtd->mtd_num, seek, eb, offs);
